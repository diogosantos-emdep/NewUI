...
namespace Emdep.Geos.Modules.Crm.ViewModels
{
    public class ActionsViewModel : NavigationViewModelBase, INotifyPropertyChanged, IDisposable
    {
        ...
        public void FillActionsListByPlant()
        {
            try
            {
                ...
                if (GeosApplication.Instance.SelectedPlantOwnerUsersList != null)
                {
                    ...
                    ActionPlanItemList = new ObservableCollection<ActionPlanItem>(CrmStartUp.GetSalesActivityRegister_V2051(null, plantOwnersIds, GeosApplication.Instance.ActiveUser.IdUser, GeosApplication.Instance.IdUserPermission, CloseDate).OrderBy(x => x.IdActionPlan).ThenBy(y => y.Number));
                    FillResponsibleList();
                }
                else
                {
        ...
        private void FillActionsListByUser()
        {
            try
            {
                ...
                if (GeosApplication.Instance.SelectedSalesOwnerUsersList != null)
                {
                    ...
                    ActionPlanItemList = new ObservableCollection<ActionPlanItem>(CrmStartUp.GetSalesActivityRegister_V2051(salesOwnersIds, null, GeosApplication.Instance.ActiveUser.IdUser, GeosApplication.Instance.IdUserPermission, CloseDate).OrderBy(x => x.IdActionPlan).ThenBy(y => y.Number));
                    Filter();
                    FillResponsibleList();
                }
                else
        ...
        private ObservableCollection<ActionPlanItem> GetAppointment(ActivityParams objActivityParams)
        {
            ...
            string ServiceUrl = "http://" + ServicePath + "/CrmRestService.svc" + "/GetAppointmentActivities";
        ...
        private void FillActionsList()
        {
            try
            {
                ...
                ActionPlanItemList = new ObservableCollection<ActionPlanItem>(CrmStartUp.GetSalesActivityRegister_V2051(null, null, GeosApplication.Instance.ActiveUser.IdUser, GeosApplication.Instance.IdUserPermission, CloseDate).OrderBy(x => x.IdActionPlan).ThenBy(y => y.Number));
                Filter();
                FillResponsibleList();
                if (DXSplashScreen.IsActive && !GeosApplication.Instance.IsLoadOneTime) { DXSplashScreen.Close(); }
                    ...
                    List<ActionPlanItem> List = new List<ActionPlanItem>(ActionPlanItemList);
                        ...
                        DevExpress.Spreadsheet.Worksheet wsDl = workbook.Worksheets[1];
                            ...
                            for (int i = 0; i < List.Count; i++)
                            {
                                ...
                                wsDl.Cells[i + 2, 3].Value = List[i].Group;
                                wsDl.Cells[i + 2, 4].Value = List[i].Plant;
                                wsDl.Cells[i + 2, 5].Value = List[i].Scope;
                                wsDl.Cells[i + 2, 6].Value = List[i].Title;
                                string comments = "";
                                DateTimeFormatInfo format = DevExpress.Data.Mask.DateTimeMaskManager.GetGoodCalendarDateTimeFormatInfo(CultureInfo.CurrentCulture);
                                ...
                                wsDl.Cells[i + 2, 7].Value = comments;
                                wsDl.Cells[i + 2, 8].Value = List[i].Reporter;
                                wsDl.Cells[i + 2, 9].Value = List[i].CreationDate;
                                wsDl.Cells[i + 2, 10].Value = List[i].ModificationDate;
                                wsDl.Cells[i + 2, 11].Value = List[i].CurrentDueDate;
                                wsDl.Cells[i + 2, 12].Value = List[i].CloseDate;
                                wsDl.Cells[i + 2, 13].Value = List[i].Assignee;
                            ...
                            for (int i = 0; i < List.Count; i++)
                            {
                                ...
                                wsSAP.Cells[row, 3].Value = List[i].Group;
                                wsSAP.Cells[row, 4].Value = List[i].Plant;
                                string comments = "";
                                ...
                                wsSAP.Cells[row, 5].SetRichText(richText);
                                //wsSAP.Cells[row, 5].Value =  List[i].Title  + Environment.NewLine.ToString() + comments;
                                wsSAP.Cells[row, 6].Value = List[i].ModificationDate;
                                wsSAP.Cells[row, 7].Value = List[i].Assignee;

                                if ((List[i].CurrentDueDate != null && ((DateTime)(List[i].CurrentDueDate) < Emdep.Geos.UI.Common.GeosApplication.Instance.ServerDateTime.Date)))
                                {
                                    ...
                                    wsSAP.Cells[row, 8].SetRichText(richText_DueDate);
                                }
                                else
                                {
                                    wsSAP.Cells[row, 8].Value = List[i].CurrentDueDate.ToShortDateString();
                                }
                                wsSAP.Cells[row, 9].Alignment.Vertical = SpreadsheetVerticalAlignment.Center;
                                wsSAP.Cells[row, 9].Alignment.WrapText = true;
                                wsSAP.Cells[row, 9].Value = List[i].Status.Value;
                                wsSAP.Cells[row, 9].Fill.BackgroundColor = ColorTranslator.FromHtml(List[i].Status.HtmlColor);
                                wsSAP.Cells[row, 10].CopyFrom(wsSAP.Cells[9, 10]);
                                row = row + 1;
        ...
        public void UpdateMultipleRowsActionsGridCommandAction(object obj)
        {
            try
            {
                ...
                cellIdStatus = StatusList.Where(sl => sl.IdLookupValue == ActionPlanMultipleCellEditHelper.IdStatus).Select(u => u.IdLookupValue).FirstOrDefault();
                ...
                foreach (ActionPlanItem item in foundRow)
                {
                    ...
                    TempActionPlanItemList.Add(CrmStartUp.GetActionPlanItem_V2043(_ActionPlanItem.IdActionPlanItem));
                    ...
                    IsActionSave = CrmStartUp.UpdateActionPlanItemForGrid(_ActionPlanItem);
                }
        ...
        private void FillResponsibleList()
        {
            try
            {
                ...
                {
                    var IdSite = Convert.ToInt32(item.IdSite);
                    List<People> responsibles = new List<People>(CrmStartUp.GetActionPlanItemResponsible(IdSite));
        ...
        private void CustomShowFilterPopup(FilterPopupEventArgs e)
        {
            try
            {
                ...
                if (e.Column.FieldName == "Status.IdLookupValue")
                {
                    ...
                    foreach (ActionPlanItem item in ActionPlanItemList)
                    {
                        ...
                        if (!filterItems.Any(x => Convert.ToString(((CustomComboBoxItem)x).DisplayValue) == StatusValue))
                        {
                            ...
                            customComboBoxItem.EditValue = item.Status.IdLookupValue;
