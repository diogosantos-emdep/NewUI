...
namespace Emdep.Geos.Services.Web.Workbench
{
    ...
    public class WorkbenchStartUp : IWorkbenchStartUp
    {
        ...
        public UserLoginEntry AddUserLoginEntry_V2210(UserLoginEntry userLoginEntry)
        {
            try
            {
                UserManager usermgr = new UserManager();
                string workbenchContextConnectionString = ConfigurationManager.ConnectionStrings["WorkbenchContext"].ConnectionString;
                return usermgr.AddUserLoginEntry_V2210(workbenchContextConnectionString, userLoginEntry);
            }
            catch (EntityException ee)
            {
                ServiceException exp = new ServiceException();
        ...
        public List<GeosModule> GetUserModulesPermissions_V2220(int idUser)
        {
            ...
            {
                GeosWorkbenchVersionsManager geosModulemgr = new GeosWorkbenchVersionsManager();
                geosModules = geosModulemgr.GetUserModulesPermissions_V2220(idUser);
            }
            catch (EntityException ee)
            {
                ServiceException exp = new ServiceException();
        ...
        //[rdixit][GEOS2-3741][19.06.2023]
        public void DeleteUserProfileImage_V2400(User user)
        {
            try
            {
                if (File.Exists(Properties.Settings.Default.UserProfileImage + user.Login + ".jpg"))
                ...
                #region [rdixit][GEOS2-3741][19.06.2023]
                if (File.Exists(Properties.Settings.Default.UserProfileImage + user.Login + ".png"))
                {
                    File.Delete(Properties.Settings.Default.UserProfileImage + user.Login + ".png");
                }
                if (File.Exists(Properties.Settings.Default.UserProfileImage + "/_tmp/" + user.Login + "_temp.png"))
                {
                    File.Delete(Properties.Settings.Default.UserProfileImage + "/_tmp/" + user.Login + "_temp.png");
                }
                #endregion
                UserManager usermgr = new UserManager();
                usermgr.UpdateUserIsValidateNull(user);
        ...
        public List<GeosSettings> GetSelectedGeosSettings_V2420(string SettingName)
        {
            ...
            try
            {
                ...
                UserManager usermgr = new UserManager();
                geosSettings = usermgr.GetSelectedGeosSettings_V2420(workbenchConnectionString, SettingName);
            }
            catch (EntityException ee)
            {
                ServiceException exp = new ServiceException();
        ...
        //[GEOS2-5021][10.11.2023][rdixit]
        public FileReturnMessage DeleteFile_V2450(FileDetail fileDetail)
        {
            FileReturnMessage fileReturnMessage = new FileReturnMessage();
            try
            {
                if (!string.IsNullOrEmpty(fileDetail.FilePath.ToString()))
                {
                    if (!fileDetail.FilePath.Contains("thumbs.db"))
                    {
                        File.Delete(fileDetail.FilePath.ToString());
                        fileReturnMessage.IsFileActionPerformed = true;
                    }
        ...
        //[GEOS2-5021][10.11.2023][rdixit]
        public FileReturnMessage CheckFileExistOrNot_V2450(FileDetail fileDetail)
        {
            FileReturnMessage fileReturnMessage = new FileReturnMessage();
        ...
        //[GEOS2-5021][10.11.2023][rdixit]
        public FileReturnMessage CheckDirectoryExistOrNot_V2450(FileDetail fileDetail)
        {
            FileReturnMessage fileReturnMessage = new FileReturnMessage();
            try
            {
        ...
        //[GEOS2-5021][10.11.2023][rdixit]
        public FileReturnMessage FileUpload_V2450(FileDetail fileDetail)
        {
        ...
        public bool CopyDirectory(string sourcePath, string destinationPath)
        {
            ...
            try
            {
                if (Directory.Exists(sourcePath))
                {
                    ...
                    foreach (var fileOrDirectory in files)
                    {
                        string newDestinationPath = Path.Combine(destinationPath, Path.GetFileName(fileOrDirectory));

                        if (Directory.Exists(fileOrDirectory))
                        {
                            // If it's a directory, recursively copy it
                            CopyDirectory(fileOrDirectory, newDestinationPath);
                        }
                        else
                        {
                            // If it's a file, copy it
                            File.Copy(fileOrDirectory, newDestinationPath, true);
                        }
                    }
                    result = true;
                }
        ...
        //[rdixit][28.11.2023][GEOS2-5088]
        public FileReturnMessage FileUpload_V2460(FileDetail fileDetail)
        {
        ...
        //[rdixit][29.01.2024]
        public FileReturnMessage FileUpload_V2480(FileDetail fileDetail)
        {
        ...
        public bool CopyDirectory_V2480(string sourcePath, string destinationPath)
        {
            bool result = false;
            try
            {
                ...
                {                  
                    ...
                    foreach (var fileOrDirectory in files)
                    {
                        ...
                        if (Directory.Exists(fileOrDirectory))
                        {                          
                            CopyDirectory_V2480(fileOrDirectory, newDestinationPath);
                        }
                        else
                        {
                            if (!FileIsInUse(fileOrDirectory))//Check file which are not beign used by other process will copy to the main server
                            {
                                File.Copy(fileOrDirectory, newDestinationPath, true);
                            }
                        }
        ...
        public List<DomainUser> GetUserDataFromTheActiveDirectory_V2490()
        {
            ...
            {
                UserManager mgr = new UserManager();
                return mgr.GetUserDataFromTheActiveDirectory_V2490();
            }
            catch (Exception ex)
            {
                ServiceException exp = new ServiceException();
        ...
        public bool UserAuthenticate(string userName, string password)
        {
            try
            {
                UserManager mgr = new UserManager();
                string workbenchConnectionString = ConfigurationManager.ConnectionStrings["WorkbenchContext"].ConnectionString;
                GeosAppSetting Setting = mgr.GetGeosAppSettings(123, workbenchConnectionString);
                return mgr.UserAuthenticate(userName, password, Setting.DefaultValue, Properties.Settings.Default.DomainName);
            }
            catch (Exception ex)
            {
                ServiceException exp = new ServiceException();
        ...
        public List<DomainUser> GetUserDataFromTheActiveDirectory_V2610()
        {
            ...
            {
                UserManager mgr = new UserManager();
                return mgr.GetUserDataFromTheActiveDirectory_V2610();
            }
            catch (Exception ex)
            {
                ServiceException exp = new ServiceException();
        ...
        public User GetUserByLoginName_V2680(string login)
        {
            User user = null;
            try
            {
                UserManager usermgr = new UserManager();
                string connectionString = ConfigurationManager.ConnectionStrings["CrmContext"].ConnectionString;
                user = usermgr.GetUserByLoginName_V2680(login, connectionString);
            }
            catch (EntityException ee)
            {
                ServiceException exp = new ServiceException();
        ...
        public User GetUserByCode_V2680(string code)
        {
            User user = null;
            try
            {
                UserManager mgr = new UserManager();
                string connectionString = ConfigurationManager.ConnectionStrings["CrmContext"].ConnectionString;
                user = mgr.GetUserByCode_V2680(code, connectionString);
            }
            catch (EntityException ee)
            {
                ServiceException exp = new ServiceException();
        ...
        public List<Company> GetAllCompanyByUserId_V2680(Int32 userId)
        {
            List<Company> companyList = null;
            try
            {
                UserManager mgr = new UserManager();
                string connectionString = ConfigurationManager.ConnectionStrings["WorkbenchContext"].ConnectionString;
                companyList = mgr.GetAllCompanyByUserId_V2680(userId, connectionString);
            }
            catch (EntityException ee)
            {
                ServiceException exp = new ServiceException();
        ...
        public List<GeosModule> GetUserModulesPermissions_V2690(int idUser)
        {
            List<GeosModule> geosModules = null;
            try
            {
                GeosWorkbenchVersionsManager geosModulemgr = new GeosWorkbenchVersionsManager();
                string connectionString = ConfigurationManager.ConnectionStrings["WorkbenchContext"].ConnectionString;
                geosModules = geosModulemgr.GetUserModulesPermissions_V2690(idUser, connectionString);
            }
            catch (EntityException ee)
            {
                ServiceException exp = new ServiceException();
        ...
        public List<Company> GetPlantListwithISO_V2690(string PlantIds)
        {
            try
            {
                UserManager mgr = new UserManager();
                string connectionString = ConfigurationManager.ConnectionStrings["CrmContext"].ConnectionString;
                return mgr.GetPlantListwithISO_V2690(connectionString, PlantIds);
            }
            catch (Exception ex)
            {
                ServiceException exp = new ServiceException();
