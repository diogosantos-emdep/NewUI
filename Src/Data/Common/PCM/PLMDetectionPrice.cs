...
{
    [DataContract]
    public class PLMDetectionPrice : ModelBase, IDisposable
    {
        #region Declaration
        ...
        {
            PLMDetectionPrice pLMDetectionPrice = (PLMDetectionPrice)this.MemberwiseClone();
            if (Status != null)
                pLMDetectionPrice.Status = (LookupValue)this.Status.Clone();

            if (currency != null)
                pLMDetectionPrice.currency = (Currency)this.currency.Clone();

            if (Rule != null)
                pLMDetectionPrice.Rule = (LookupValue)this.Rule.Clone();


            return pLMDetectionPrice;
        }
