    ...
    public class EmployeeDocumentViewModel : INotifyPropertyChanged
    {
        ...
        public void OpenPdfByEmployeeCode(string empCode, object empRecord, bool isJobCode = false)
        {
            ...
            catch (ServiceUnexceptedException ex)
            {
                ...
                GeosApplication.Instance.Logger.Log("Get an error in OpenPdfByEmployeeCode() Method - ServiceUnexceptedException " + ex.Message, category: Category.Info, priority: Priority.Low);
        ...
        public void OpenPdfFileTraineeResultByEmployeeCode(string empCode, object traineeRecord)
        {
            //string FileName = string.Empty;
            try
            {
                ...
                byte[] temp = null;
                    ...
                    if (traineeRecord is ProfessionalTrainingResults)
                    {
                        ProfessionalTrainingResults traineeDocument = (ProfessionalTrainingResults)traineeRecord;
                        FileName = traineeDocument.ResultFileName;
                        temp = traineeDocument.TraineeDocumentFileInBytes;
                        empCode = traineeDocument.EmployeeCode;
                    }
                   
                if (temp == null)
                {
                    temp = HrmService.GetEmployeeDocumentFile_V2210(empCode, traineeRecord);
                ...
                GeosApplication.Instance.Logger.Log("Method OpenPdfFileTraineeResultByEmployeeCode()....executed successfully", category: Category.Info, priority: Priority.Low);
            }
            catch (FaultException<ServiceException> ex)
            {
                GeosApplication.Instance.Logger.Log("Get an error in Method OpenPdfFileTraineeResultByEmployeeCode()...." + ex.Message, category: Category.Exception, priority: Priority.Low);
                CustomMessageBox.Show(string.Format("File Not Found..."), Application.Current.Resources["PopUpWarningColor"].ToString(), CustomMessageBox.MessageImagePath.NotOk, MessageBoxButton.OK);
            }
            catch (ServiceUnexceptedException ex)
            {
                //employeeEducationQualification.PdfFilePath
                if (DXSplashScreen.IsActive) { DXSplashScreen.Close(); }
                GeosApplication.Instance.Logger.Log("Get an error in OpenPdfFileTraineeResultByEmployeeCode() Method - ServiceUnexceptedException " + ex.Message, category: Category.Info, priority: Priority.Low);
                //GeosApplication.Instance.ExceptionHandlingOperation(ex.ExceptionType, GeosApplication.Instance.ApplicationSettings["ServicePath"].ToString(), null);
                CustomMessageBox.Show(string.Format("Could not find file {0}", FileName), Application.Current.Resources["PopUpWarningColor"].ToString(), CustomMessageBox.MessageImagePath.NotOk, MessageBoxButton.OK);
                throw;
            }
        ...
        public void OpenPdf(ProfessionalTrainingAttachments SelectedTrainingAttachmentFile, object obj)
        {
            try
            {
                ...
                if (SelectedTrainingAttachmentFile is ProfessionalTrainingAttachments)
                {
                    ProfessionalTrainingAttachments professionalTrainingAttachments = (ProfessionalTrainingAttachments)obj;
                    FileName = professionalTrainingAttachments.SavedFileName;
                    temp = professionalTrainingAttachments.ProfTrainigAttachedDocInBytes;
                }
