...
namespace Emdep.Geos.Modules.Hrm.ViewModels
{
    public class AddContactInformationViewModel : NavigationViewModelBase, INotifyPropertyChanged, IDataErrorInfo
    {
        ...
        private bool isPersonalContact;
        #endregion
        ...
        public bool IsPersonalContact
        {
            get { return isPersonalContact; }
            set
            {
                isPersonalContact = value;
                OnPropertyChanged(new PropertyChangedEventArgs("IsPersonalContact"));
            }
        }
        string RemoveEmailGeosAppSetting { get; set; }
        ...
        private void AddContactInformation(object obj)
        {
            ...
            {
                GeosApplication.Instance.Logger.Log("Get an error in Method AddContactInformation() " + ex.Message + GeosApplication.createExceptionDetailsMsg(ex), category: Category.Exception, priority: Priority.Low);
            }
        }
        ...
        private void SelectedIndexContactTypeCommandAction(object obj)
        {
            try
            {
                GeosApplication.Instance.Logger.Log("Method SelectedIndexContactTypeCommandAction()...", category: Category.Info, priority: Priority.Low);
                if (SelectedIndexContactType == 0) return;

                if (ContactTypeList != null && ContactTypeList[SelectedIndexContactType].IdLookupValue == 88 && IsPersonalContact == false)
                {
                    IsTextbox = false;
                    IsCombobox = true;
                    ...
                    if (GeosApplication.Instance.DomainUsers == null)
                    {
                        ...
                        //Shubham[skadam] https://helpdesk.emdep.com/browse/IESD-94486 HRM EAES - Can't introduce new employees 17 01 2024
                        IWorkbenchStartUp WorkbenchServiceNew = null;
                        try
                        {
                             WorkbenchServiceNew = new WorkbenchServiceController(GeosApplication.Instance.ApplicationSettings["ServicePath"].ToString());

                           // WorkbenchServiceNew = new WorkbenchServiceController("localhost:6699");

                            // GeosApplication.Instance.DomainUsers = WorkbenchServiceNew.GetUserDataFromTheActiveDirectory();
                            //[Sudhir.Jangra][GEOS2-3418]
                            GeosApplication.Instance.DomainUsers = WorkbenchServiceNew.GetUserDataFromTheActiveDirectory_V2490();

                        }
                        catch (FaultException<ServiceException> ex)
                        {
                            GeosServiceProvider geosServiceProvider = GeosApplication.Instance.GeosServiceProviders.GeosServiceProvider.Where(w => w.Name.Trim().ToUpper().Equals("EAES".ToUpper())).FirstOrDefault();
                            WorkbenchServiceNew = new WorkbenchServiceController(geosServiceProvider.ServiceProviderUrl);
                            // GeosApplication.Instance.DomainUsers = WorkbenchServiceNew.GetUserDataFromTheActiveDirectory();
                            //[Sudhir.Jangra][GEOS2-3418]
                            GeosApplication.Instance.DomainUsers = WorkbenchServiceNew.GetUserDataFromTheActiveDirectory_V2490();
                        }
                        ...
                        //GeosApplication.Instance.DomainUsers = WorkbenchServiceNew.GetUserDataFromTheActiveDirectory(); // GeosApplication.Instance.GetUserDataFromTheActiveDirectory();

                        if (DXSplashScreen.IsActive) { DXSplashScreen.Close(); }
                    }

                    SuggestionsList = GeosApplication.Instance.DomainUsers.Where(x => !string.IsNullOrEmpty(x.Company) && x.Email.EndsWith("@emdep.com")).Select(x => x.Email).ToList();

                    SuggestionsList = SuggestionsList.Where(s => !string.IsNullOrWhiteSpace(s)).Distinct().ToList();

                    #region [rdixit][28.07.2023][GEOS2-4396]
                    RemoveEmailGeosAppSetting = CrmStartUp.GetGeosAppSettings(104).DefaultValue;
                    string[] RemoveEmailList = RemoveEmailGeosAppSetting.Split(';');
                    foreach (var pattern in RemoveEmailList)
                    {
                        SuggestionsList = SuggestionsList.Where(email => !IsEmailMatch(email, pattern.Trim())).ToList();
                    }
                    #endregion

                    //#region GEOS2-3418 [Sudhir.Jangra]
                    //RemoveEmailGeosAppSetting = CrmStartUp.GetGeosAppSettings(117).DefaultValue;
                    //string[] RemoveEmailListdata = RemoveEmailGeosAppSetting.Split(',');
                    
                    //SuggestionsList = SuggestionsList.Where(email => !ContainsExcludedKeyword(email, RemoveEmailListdata)).ToList();
                ...
                GeosApplication.Instance.Logger.Log("Method SelectedIndexContactTypeCommandAction()....executed successfully", category: Category.Info, priority: Priority.Low);
            }
            catch (FaultException<ServiceException> ex)
            {
                if (DXSplashScreen.IsActive) { DXSplashScreen.Close(); }
        ...
        //[Sudhir.Jangra][GEOS2-3418]
        static bool ContainsExcludedKeyword(string email, string[] keywords)
        {
            foreach (var keyword in keywords)
            {
                if (!string.IsNullOrEmpty(keyword.Trim()))
                {
                    if (email.IndexOf(keyword.Trim(), StringComparison.OrdinalIgnoreCase) != -1)
                    {
                        return true;
                    }
                }
        ...
        //[rdixit][28.07.2023][GEOS2-4396]
        static bool IsEmailMatch(string email, string pattern)
        {
            pattern = Regex.Escape(pattern).Replace("\\*", ".*");
            bool IsTrue = Regex.IsMatch(email, $"^{pattern}$", RegexOptions.IgnoreCase);
            return IsTrue;
        }
