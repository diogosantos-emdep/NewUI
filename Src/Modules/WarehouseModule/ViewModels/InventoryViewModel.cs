...
namespace Emdep.Geos.Modules.Warehouse.ViewModels
{
    class InventoryViewModel : ViewModelBase, INotifyPropertyChanged
    {
        ...
        public DateTime? DateTimeValue
        {
            ...
            set { _dateTimeValue = value == new DateTime(0) ? null : value; }
        }
        ...
        /// [003][vsana][22-11-2020][GEOS2-2426]AutoSort for the new locations created 
        /// [004][cpatil][22-11-2021][GEOS2-3476]Inventory audit for some reason appear duplicated entries
        /// </summary>
        ...
        private void ScanBarcodeAction(TextCompositionEventArgs obj)
            ...
            try
            {
                if (obj.Text == "\r" && IsLocationScanned == false)
                {
                    ...
                    {
                        //[GEOS2-3964][rsarode][14-12-2022]
                        ObservableCollection<InventoryAuditLocation> tmpWarehouseInventoryAudit = new ObservableCollection<InventoryAuditLocation>();
                        //[rdixit][09.12.2022][GEOS2-3962] New service created to validate Location                       
                        bool IsLocExistInScope = WarehouseService.IsLocationExistForSelectedWarehouseInventoryAudit(WarehouseCommon.Instance.Selectedwarehouse.IdWarehouse,LocationName, SelectedWarehouseInventoryAudit.IdWarehouseInventoryAudit);
                        if (IsLocExistInScope)
                        {
                            ...
                            InventoryMaterials = new ObservableCollection<InventoryMaterial>(WarehouseService.GetInventoryArticleByIdWarehouseLocation_V2520(WarehouseCommon.Instance.Selectedwarehouse, ScannedWarehouseLocation.IdWarehouseLocation));
                            warehouseInventoryAuditItems = new List<WarehouseInventoryAuditItem>(WarehouseService.GetWarehouseInventoryAuditItemsByInventoryAudit_V2090(WarehouseCommon.Instance.Selectedwarehouse, SelectedWarehouseInventoryAudit, ScannedWarehouseLocation));

                            InventoryMaterials = new ObservableCollection<InventoryMaterial>(InventoryMaterials.OrderBy(x => x.WarehouseDeliveryNote.DeliveryDateNullable));
                            ...
                            if (warehouseInventoryAuditItems.Count > 0)
                            {
                                foreach (WarehouseInventoryAuditItem auditItem in warehouseInventoryAuditItems)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if (auditItem.WarehouseDeliveryNoteItem != null)
                        ...
                        else
                        {
                            ...
                            WrongItem = "Wrong location. It is not included in the current inventory audit";
                ...
                else if (obj.Text == "\r" && IsLocationScanned == true)
                {
                    ...
                    //[rdixit][16.12.202][GEOS2-3964]
                    Int64 _idWareHouseDeliveryNote = Convert.ToInt64(BarcodeStr.Substring(3, 8));
                    InventoryMaterial _InventoryMaterials = InventoryMaterials.FirstOrDefault(x => x.IdWareHouseDeliveryNoteItem == _idWareHouseDeliveryNote);                                                            
                    if (_InventoryMaterials != null)
                    {
                        //[rdixit][09.12.2022][GEOS2-3962] New service created to validate Article  
                        bool isArticleExistinScope = WarehouseService.IsArticleExistForSelectedWarehouseInventoryAudit(_InventoryMaterials.IdArticle, SelectedWarehouseInventoryAudit.IdWarehouseInventoryAudit);
                        if (!isArticleExistinScope)
                        {
                            IsWrongItemErrorVisible = Visibility.Visible;
                            WrongItem = "Wrong Article " + _InventoryMaterials.Reference + " . It is not included in the current inventory audit scope";
                            GeosApplication.Instance.PlaySound(WarehouseCommon.Instance.BeepNotOkFilePath);
                            BarcodeStr = string.Empty;
                            if (DXSplashScreen.IsActive) { DXSplashScreen.Close(); }
                            return;
                    ...
                    if (isProperBarcode)
                    {
                        ...
                        if (!InventoryMaterials.Any(x => x.IdWareHouseDeliveryNoteItem == _idWareHouseDeliveryNoteItem))
                        {
                            ...
                            //[rdixit][16.12.202][GEOS2-3964]
                            _InventoryMaterials = tempInventoryMaterials.FirstOrDefault(x => x.IdWareHouseDeliveryNoteItem == _idWareHouseDeliveryNote);
                            bool isArticleExistinScope = WarehouseService.IsArticleExistForSelectedWarehouseInventoryAudit(_InventoryMaterials.IdArticle, SelectedWarehouseInventoryAudit.IdWarehouseInventoryAudit);
                            if (isArticleExistinScope)
                            {
                                WrongItem = string.Empty;
                            ...
                            {
                                IsWrongItemErrorVisible = Visibility.Visible;
                                WrongItem = "Wrong Article " + _InventoryMaterials.Reference + " . It is not included in the current inventory audit";
                                GeosApplication.Instance.PlaySound(WarehouseCommon.Instance.BeepNotOkFilePath);
                                BarcodeStr = string.Empty;
                                if (DXSplashScreen.IsActive) { DXSplashScreen.Close(); }
                                return;
            ...
            {
                GeosApplication.Instance.Logger.Log("Get an error in ScanBarcodeAction Method ScanReference() Method " + ex.Detail.ErrorMessage, category: Category.Exception, priority: Priority.Low);
                CustomMessageBox.Show(GeosApplication.Instance.ExceptionHandlingOperationString(ex.Detail, null), "Red", CustomMessageBox.MessageImagePath.NotOk, MessageBoxButton.OK);
            }
            catch (ServiceUnexceptedException ex)
            {
                GeosApplication.Instance.Logger.Log("Get an error in ScanBarcodeAction Method ScanReference() Method " + ex.Message, category: Category.Exception, priority: Priority.Low);
        ...
        private void AcceptAction(object obj)
        {
            try
            {
                ...
                foreach (InventoryMaterial inventoryMaterial in InventoryMaterials)
                {
                    ...
                    //Task GEOS2-2779 // When doing an inventory audit, the items scanned correctly, means, than no difference has been found, they should be approved automatically after press Accept and move to the next location
                    if (inventoryMaterial.RemainingQty == 0)
                    {
                        WarehouseInventoryAuditItem.IdApprover = GeosApplication.Instance.ActiveUser.IdUser;
                        WarehouseInventoryAuditItem.IsOK = 1; 
                    }
                ...
                {
                    //AddUpdateWarehouseInventoryAuditItems_V2090 version updated with AddUpdateWarehouseInventoryAuditItems_V2340 //[rdixit][09.12.2022][GEOS2-3962]                                        
					WarehouseService.AddUpdateWarehouseInventoryAuditItems_V2590(WarehouseCommon.Instance.Selectedwarehouse, tempwarehouseInventoryAuditItemsList);
                }
