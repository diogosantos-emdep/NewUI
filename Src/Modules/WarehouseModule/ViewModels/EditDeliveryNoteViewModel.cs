...
namespace Emdep.Geos.Modules.Warehouse.ViewModels
{
    public class EditDeliveryNoteViewModel : NavigationViewModelBase, IDataErrorInfo, INotifyPropertyChanged, IDisposable
    {
       ...
       // IWarehouseService WarehouseService = new WarehouseServiceController("localhost:6699");
        ...
        List<WarehouseDeliveryNoteItem> newItemList;
        bool isArticleView;

        List<long> lockChangedList;//[Sudhir.Jangra]
        #endregion
        ...
        public List<WarehouseDeliveryNoteItem> NewItemList
        {
            get { return newItemList; }
            set
            {
                newItemList = value;
                OnPropertyChanged(new PropertyChangedEventArgs("NewItemList"));
            }
        }

        public bool IsArticleView
        {
            get { return isArticleView; }
            set
            {
                isArticleView = value;
                OnPropertyChanged(new PropertyChangedEventArgs("IsArticleView"));
            }
        }

        public List<long> LockChangedList
        {
            get { return lockChangedList; }
            set
            {
                lockChangedList = value;
                OnPropertyChanged(new PropertyChangedEventArgs("LockChangedList"));
            }
        }
        #endregion
        ...
        public ICommand LockUnlockCommand { get; set; }//[Sudhir.Jangra][GEOS2-4543]
        ...
        public EditDeliveryNoteViewModel()
        {
            try
            {
                ...
                LockUnlockCommand = new RelayCommand(new Action<object>(LockUnlockCommandAction));//[Sudhir.Jangra][GEOS2-4543]

                LockChangedList = new List<long>();
            ...
            try
            {
                if (parameter != null)
                {
                    ...
                    if (deliveryNotePrintPreviewViewModel.IsAccept)
                    {
                        ...
                        #endregion
                        #region GEOS2-6008
                        //GEOS2-6008 Codificar proveedores 10 10 2024
                        ...
                        try
                        {
                            EmployeeCode= WarehouseService.GetEmployeeCodeByIdUser(GeosApplication.Instance.ActiveUser.IdUser); //[GEOS2-4014][rdixit][15.12.2022]
                            warehouseDeliveryNote = WarehouseService.GetLabelPrintDetails_V2350(deleteItem.WarehousePurchaseOrderItem.Article.IdArticle, 
                                WarehouseCommon.Instance.Selectedwarehouse, deleteItem.IdWarehouseDeliveryNoteItem);
                            ...
                            else if (!string.IsNullOrEmpty(warehouseDeliveryNote.WarehouseDeliveryNoteItems[0].ManufacturersByArticle[0].Manufacturer.IdManufacturer.ToString()))
                            {
                                producer = "PROD" + warehouseDeliveryNote.WarehouseDeliveryNoteItems[0].ManufacturersByArticle[0].Manufacturer.IdManufacturer.ToString().PadLeft(6, '0');
                            }
                            else
                            {
                            ...
                            else if (!string.IsNullOrEmpty(WarehouseDeliveryNote.WarehousePurchaseOrder.ArticleSupplier.IdArticleSupplier.ToString()))
                            {
                                supplier = "SUPL" + WarehouseDeliveryNote.WarehousePurchaseOrder.ArticleSupplier.IdArticleSupplier.ToString().PadLeft(6, '0');
                            }
                            else if (!string.IsNullOrEmpty(warehouseDeliveryNote.SupplierComplaint.Supplier.Code))
                            {
                        ...
                        {
                            deliveryNoteItemPrintDetails.Name = EmployeeCode;
                        }
                        deliveryNoteItemPrintDetails.SupplierName = ArticleSupplier;
                        if (!string.IsNullOrEmpty(supplier))
                        {
                            deliveryNoteItemPrintDetails.SupplierName = supplier;
                        }
                        deliveryNoteItemPrintDetails.Code = DeliveryNoteCode;
                        if (deleteItem.Producer != null)
                        {
                            ...
                            {
                                deliveryNoteItemPrintDetails.ProducerName = producer;
                            }
                            deliveryNoteItemPrintDetails.MadeIn = deleteItem.Producer.Country.Name;
                            if (deleteItem.Producer.CountryGroup != null)
                            {
        ...
        public void Init(WarehouseDeliveryNote warehouseDeliveryNote)
        {
            try
            {
                ...
                if (warehouseDeliveryNote.WarehousePurchaseOrder.ArticleSupplier != null)
                {
        ...
        public void SaveDeliveryNote(object obj)
        {
            try
            {
                ...
                if (WarehouseDeliveryNoteItems != null)
                {
                    List<WarehouseDeliveryNoteItem> changedData = WarehouseDeliveryNoteItems.Where(item => LockChangedList.Contains(item.IdWarehouseDeliveryNoteItem)).ToList();

                    foreach (var item in changedData)
                    {
                        bool idUpdated = WarehouseService.IsUpdatedLockPropertyForDeliveryNote(item.IdWarehouseDeliveryNoteItem, item.IsLocked);
                    }

                    List<WarehouseDeliveryNoteItem> temp = new List<WarehouseDeliveryNoteItem>(WarehouseDeliveryNoteItems.Where(x => x.IsLocked == true && !string.IsNullOrEmpty(x.ReasonComment)));
                    if (temp.Count > 0)
                    {
                        NewItemList = new List<WarehouseDeliveryNoteItem>(temp);
                        bool isUpdated = WarehouseService.IsLockPropertyForDeliveryNoteUpdated_V2510(temp);

                       // CustomMessageBox.Show(string.Format(System.Windows.Application.Current.FindResource("DeliveryNoteReasonSavedMessage").ToString()), "Green", CustomMessageBox.MessageImagePath.Ok, MessageBoxButton.OK);
                        IsDeliveryNoteSaved = true;

                    }
                    List<WarehouseDeliveryNoteItem> data = new List<WarehouseDeliveryNoteItem>(WarehouseDeliveryNoteItems.Where(x => x.IsLocked == false));

                    if (data.Count > 0)
                    {

                        data.ForEach(x => x.IdLockedReason = 0);
                        data.ForEach(x => x.ReasonComment = string.Empty);
                        NewItemList = new List<WarehouseDeliveryNoteItem>(data);
                        bool isUpdated = WarehouseService.IsLockPropertyForDeliveryNoteUpdated_V2510(data);
                    ...
                    if (IsArticleView && temp.Count == 0)
                    {
                        WarehouseDeliveryNoteItem temp1 = new WarehouseDeliveryNoteItem();
                        temp1.IsLocked = false;
                        temp1.ReasonComment = "";
                        NewItemList = new List<WarehouseDeliveryNoteItem>();
                        NewItemList.Add(temp1);
                        IsDeliveryNoteSaved = true;
        ...
        private void LockUnlockCommandAction(object obj)
        {
            try
            {
                GeosApplication.Instance.Logger.Log("Method LockUnlockCommandAction...", category: Category.Info, priority: Priority.Low);
                bool isLocked = false;
                if (SelectedGridRow.IsLocked == true)
                {
                    isLocked = false;
                }
                else
                ...
                if (isLocked)
                {
                    ...
                    {
                        DXSplashScreen.Show<SplashScreenView>();
                        DeliveryNoteReasonView deliveryNoteReasonView = new DeliveryNoteReasonView();
                        DeliveryNoteReasonViewModel deliveryNoteReasonViewModel = new DeliveryNoteReasonViewModel();
                        deliveryNoteReasonView.DataContext = deliveryNoteReasonViewModel;
                        EventHandler handle = delegate { deliveryNoteReasonView.Close(); };
                        deliveryNoteReasonViewModel.RequestClose += handle;
                        deliveryNoteReasonView.ShowDialogWindow();

                        if (deliveryNoteReasonViewModel.IsSave)
                        {
                            SelectedGridRow.IdLockedReason = deliveryNoteReasonViewModel.NewReason.IdLockedReason;
                            SelectedGridRow.ReasonComment = deliveryNoteReasonViewModel.NewReason.ReasonComment;
                            WarehouseDeliveryNoteItems
                                .Where(x => x.IdWarehouseDeliveryNote == SelectedGridRow.IdWarehouseDeliveryNoteItem)
                                .ToList()
                                .ForEach(a =>
                                {
                                    a.IdLockedReason = deliveryNoteReasonViewModel.NewReason.IdLockedReason;
                                    a.ReasonComment = deliveryNoteReasonViewModel.NewReason.ReasonComment;
                                });

                            if (SelectedGridRow is WarehouseDeliveryNoteItem)
                            {
                                WarehouseDeliveryNoteItem warehouseDeliveryNoteItem = (WarehouseDeliveryNoteItem)SelectedGridRow;
                                SelectedGridRow.IsLocked = true;
                                LockChangedList.Add(SelectedGridRow.IdWarehouseDeliveryNoteItem);
                                //  bool idUpdated = WarehouseService.IsUpdatedLockPropertyForDeliveryNote(SelectedGridRow.IdWarehouseDeliveryNoteItem, SelectedGridRow.IsLocked);
                    ...
                    {
                        if (DXSplashScreen.IsActive && !GeosApplication.Instance.IsLoadOneTime) { DXSplashScreen.Close(); }
                        GeosApplication.Instance.Logger.Log("Get an error in Method LockUnlockCommandAction()...." + ex.Message, category: Category.Exception, priority: Priority.Low);
                    }
                ...
                else
                {
                    ...
                    {
                        WarehouseDeliveryNoteItem warehouseDeliveryNoteItem = (WarehouseDeliveryNoteItem)SelectedGridRow;
                        SelectedGridRow.IsLocked = false;
                        LockChangedList.Add(SelectedGridRow.IdWarehouseDeliveryNoteItem);
                        // bool idUpdated = WarehouseService.IsUpdatedLockPropertyForDeliveryNote(SelectedGridRow.IdWarehouseDeliveryNoteItem, SelectedGridRow.IsLocked);
                ...
                GeosApplication.Instance.Logger.Log("Method LockUnlockCommandAction executed successfully.", category: Category.Info, priority: Priority.Low);
            }
            catch (FaultException<Services.Contracts.ServiceException> ex)
            {
                if (DXSplashScreen.IsActive) { DXSplashScreen.Close(); }
                GeosApplication.Instance.Logger.Log("Get an error in LockUnlockCommandAction() method " + ex.Detail.ErrorMessage + GeosApplication.createExceptionDetailsMsg(ex), category: Category.Exception, priority: Priority.Low);
                CustomMessageBox.Show(GeosApplication.Instance.ExceptionHandlingOperationString(ex.Detail, null), "Red", CustomMessageBox.MessageImagePath.NotOk, MessageBoxButton.OK);
            }
            catch (ServiceUnexceptedException ex)
            {
                if (DXSplashScreen.IsActive) { DXSplashScreen.Close(); }
                GeosApplication.Instance.Logger.Log("Get an error in LockUnlockCommandAction() Method - ServiceUnexceptedException " + ex.Message + GeosApplication.createExceptionDetailsMsg(ex), category: Category.Exception, priority: Priority.Low);
                GeosApplication.Instance.ExceptionHandlingOperation(ex.ExceptionType, GeosApplication.Instance.ApplicationSettings["ServicePath"].ToString(), null);
            }
            catch (Exception ex)
            {
                if (DXSplashScreen.IsActive) { DXSplashScreen.Close(); }
                GeosApplication.Instance.Logger.Log("Get an error in LockUnlockCommandAction() method " + ex.Message + GeosApplication.createExceptionDetailsMsg(ex), category: Category.Exception, priority: Priority.Low);
            }
        }
        #endregion
