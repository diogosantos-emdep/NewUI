                    ...
                    if (SelectedIndex == 0)
                    {
                        ...
                        if (OtsList.Any(Ot => ((Ot.NumOT.ToString().Length == 1 ? "0" + Ot.NumOT : Ot.NumOT.ToString()) == strNumOt) && (Ot.Code == workOrderCode)))
                        {
                            ...
                            try
                            {
                                ...
                                ot = WarehouseService.GetWorkOrderByIdOt_V2460(ot.IdOT, WarehouseCommon.Instance.Selectedwarehouse);
                            }
                                ...
                                do
                                {
                                    ...
                                    if (isItemsAvailable)
                                    {
                                        ...
                                        if (pickingMaterialsViewModel.UnspectedMaterialList?.Count > 0)//[rdixit][GEOS2-4930][31.10.2023]
                                            CustomMessageBox.Show(string.Format(System.Windows.Application.Current.FindResource("UnInspectedItemsAvailable").ToString(), string.Join(",", pickingMaterialsViewModel.UnspectedMaterialList.Select(r => r.Reference))), "Yellow", CustomMessageBox.MessageImagePath.Warning, MessageBoxButton.OK);
                    ...
                    else
                    {
                        try
                        {
                            ...
                            if (idOT > 0)
                            {
                                ...
                                //Service Updated from GetWorkOrderByIdOt_V2450 to GetWorkOrderByIdOt_V2460 by [rdixit][30.11.2023][GEOS2-5068]
                                ot = WarehouseService.GetWorkOrderByIdOt_V2460(idOT, WarehouseCommon.Instance.Selectedwarehouse);
                                if (!IsRefund)
                                {
                                    WorkOrderBarcodeErrorVisibility = Visibility.Hidden;
                                    bool FollowFIFO = true;
