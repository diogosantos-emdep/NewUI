...
namespace Emdep.Geos.Modules.Crm.ViewModels
{
    public class LeadsViewModel : NavigationViewModelBase, INotifyPropertyChanged, IDisposable //, ISupportServices
    {
        ...
        //[004][soumitra.kulkarni][16/05/2025][GEOS-6583]CRM Opportunity Timeline conditional formating filter issue.
        ...
        //public ICrmService CrmStartUp = new CrmServiceController("localhost:6699");
        #endregion // Services
        ...
        //[nsatpute][19.06.2025][GEOS2-6689]
        private static readonly string GridStatePath = Path.Combine(
    GeosApplication.Instance.UserSettingFolderName,
    "LeadsGridState.bin");
        ...
        public GridControl GridControl
        {
            ...
            {
                _gridControl = value;
                OnPropertyChanged(new PropertyChangedEventArgs("GridControl"));
            }
        }

        // Command to handle conditional formatting changes
        public ICommand ConditionalFormattingChangedCommand { get; set; }
        ...
        public LeadsViewModel()
        {
            try
            {
                ...
                SuccessPlantList = new List<string>(); //[nsatpute][08.07.2025][GEOS2-7205]
                ...
                ConditionalFormattingChangedCommand = new DelegateCommand<object>(ConditionalFormattingChanged);
                //[003]added
                ...
                tasks.Add(Task.Run(() => FillLeadGridDetails()));
                tasks.Add(Task.Run(() => FillConfidentialLevelList()));
                tasks.Add(Task.Run(() => FillCaroemsList()));
                tasks.Add(Task.Run(() => FillBusinessUnitList()));
                tasks.Add(Task.Run(() => FillLeadSourceList()));
                tasks.Add(Task.Run(() => FillGeosProjectsList()));
        ...
        /// [003][GEOS2-3157][cpatil][11-10-2021]Improvement in the discount option. (Set the discount null by default)
        /// [004][GEOS2-3595][cpatil][10-03-2022]Can´t update date
        /// [005][cpatil][GEOS2-5348][29.02.2024]
        /// </summary>
        ...
        public void UpdateMultipleRowsCommandAction(object obj)
        {
            try
            {
                ...
                Int32? cellIdBusinessUnit = null;
                Int32? cellIdSource = null;
                int? cellIdOEM = null;
                long? CellIdCarProject = null;
                bool IsChecked = false;
                int? cellIdSalesOwner = null;
                string discount = null;
                cellIdBusinessUnit = Convert.ToInt32(BusinessUnitList.Where(b => b.Value == MultipleCellEditHelper.BusinessUnit).Select(u => u.IdLookupValue).FirstOrDefault());
                cellIdSource = Convert.ToInt32(LeadSourceList.Where(b => b.Value == MultipleCellEditHelper.LeadSource).Select(u => u.IdLookupValue).FirstOrDefault());
                ...
                foreach (DataRow item in foundRow)
                {
                    ...
                    //[003] Changed service method and controller 
                    ...
                    TempLeadsList.Add(CrmStartUpOfferActiveSite.GetOfferDetailsById_V2200(offerId, GeosApplication.Instance.ActiveUser.IdUser, GeosApplication.Instance.IdCurrencyByRegion, GeosApplication.Instance.SelectedyearStarDate, GeosApplication.Instance.SelectedyearEndDate, offerActiveSite));
                    ...
                    if (IsEditableOffer == true)
                    {
                        ...
                        //if (OfferData.Discount != (double) dr1["Discount"])
                        //{
                        //    string DiscountOld = OfferData.Discount.ToString();
                        //    OfferData.Discount = Convert.ToSingle( dr1["Discount"]);
                        //    string DiscountNew = OfferData.Discount.ToString();
                        ...
                        if (cellIdBusinessUnit.HasValue && cellIdBusinessUnit != 0)
                        {
                        ...
                        if (cellIdSource.HasValue && cellIdSource != 0)
                        {
                        ...
                        else if (dr1["Discount"] != DBNull.Value && OfferData.DiscountNullable == null)//[004]
                        {
                        ...
                        else if (dr1["Discount"] != DBNull.Value && OfferData.DiscountNullable != (double)dr1["Discount"])//[004]
                        {
                            string DiscountOld = OfferData.DiscountNullable.ToString();
                            OfferData.DiscountNullable = Convert.ToSingle(dr1["Discount"]);
                            string DiscountNew = OfferData.DiscountNullable.ToString();
                        ...
                        try
                        {
                            ...
                            {
                                if (item22.Column != null) //[rdixit][GEOS2-5615][09.05.2024]
                                {
                ...
                foreach (Offer item in FinalLeadsList)
                {
                    ...
                    isoffersave = CrmStartUpOfferActiveSite.UpdateOfferForParticularColumn_V2490(item, item.OfferActiveSite.IdSite, GeosApplication.Instance.ActiveUser.IdUser);
                    if (isoffersave)
                        isAllSave = true;
                    else
                        isAllSave = false;
        ...
        private void RefreshLeadDetails(object obj)
        {
            ...
            var detailView = obj as TableView;
            if (detailView == null) return;
            view = detailView;
            GridControl = detailView.Grid;
            ...
            {
                RestoreFullGridStateAsync(); //[nsatpute][19.06.2025][GEOS2-6689]
        ...
        private void LeadsEditViewWindowShow(object obj)
        {
            try
            {
                ...
                if ((System.Data.DataRowView)detailView.DataControl.CurrentItem != null)
                {
                    ...
                    ActiveSite offerActiveSite = (ActiveSite)((System.Data.DataRowView)detailView.DataControl.CurrentItem).Row.ItemArray[37];
                    IList<Offer> TempLeadsList = new List<Offer>();
                    //[001] added Change Method
                    ICrmService CrmStartUpOfferActiveSite = new CrmServiceController(offerActiveSite.SiteServiceProvider);
                    //[001] Changed service method and controller
                    //TempLeadsList.Add(CrmStartUpOfferActiveSite.GetOfferDetailsById_V2090(offerId, GeosApplication.Instance.ActiveUser.IdUser, GeosApplication.Instance.IdCurrencyByRegion, GeosApplication.Instance.SelectedyearStarDate, GeosApplication.Instance.SelectedyearEndDate, offerActiveSite));
                    TempLeadsList.Add(CrmStartUpOfferActiveSite.GetOfferDetailsById_V2120(offerId, GeosApplication.Instance.ActiveUser.IdUser, GeosApplication.Instance.IdCurrencyByRegion, GeosApplication.Instance.SelectedyearStarDate, GeosApplication.Instance.SelectedyearEndDate, offerActiveSite));
                    //[001] Changed service method and controller
                    ...
                    if (leadsEditViewModel.OfferData != null)
                    {
                        ...
                        #region Final transfer offer
                        if (leadsEditViewModel.FinalTransferedOffer != null)
                        {
                            #region cut, copy & paste
                            //if (leadsEditViewModel.IsCheckedDuringTransferOffer == false)
                            //{
                            DataRow dataRow = Dttable.NewRow();

                            dataRow["Idoffer"] = leadsEditViewModel.FinalTransferedOffer.IdOffer;

                            Int64? idCategory1 = CrmStartUp.GetIdProductCategoryByIdOffer(leadsEditViewModel.FinalTransferedOffer.IdOffer, GeosApplication.Instance.CompanyList.Where(cmp => cmp.ConnectPlantId == ConnectPlantId.ToString()).FirstOrDefault().ConnectPlantConstr);

                            if (idCategory1 > 0)
                            {
                                ProductCategory objProCat = new ProductCategory();
                                objProCat = leadsEditViewModel.ListProductCategory.Where(i => i.IdProductCategory == idCategory1.Value).FirstOrDefault();
                            ...
                            dataRow["Code"] = leadsEditViewModel.FinalTransferedOffer.Code;

                            if (leadsEditViewModel.FinalTransferedOffer.Site != null)
                            {
                                if (leadsEditViewModel.FinalTransferedOffer.Site.Customers != null && leadsEditViewModel.FinalTransferedOffer.Site.Customers.Count > 0)
                                {
                                    dataRow["Group"] = leadsEditViewModel.FinalTransferedOffer.Site.Customers[0].CustomerName;
                                }

                                dataRow["ConnectPlantId"] = leadsEditViewModel.FinalTransferedOffer.OfferActiveSite.IdSite;
                                ...
                                dataRow["ActiveSite"] = leadsEditViewModel.FinalTransferedOffer.OfferActiveSite;
                                dataRow["IdSite"] = leadsEditViewModel.FinalTransferedOffer.Site.IdCompany;
                                dataRow["Plant"] = leadsEditViewModel.FinalTransferedOffer.Site.SiteNameWithoutCountry;
                                //Name.Substring(0, leadsEditViewModel.OfferData.Site.Name.IndexOf("("));

                                if (leadsEditViewModel.FinalTransferedOffer.Site.Country != null)
                                {
                                    dataRow["Country"] = leadsEditViewModel.FinalTransferedOffer.Site.Country.Name;
                                    //[GEOS2-4323][17.04.2023][rdixit]
                                    dataRow["CountryImageUrl"] = "https://api.emdep.com/GEOS/Images?FilePath=/Images/Countries/" + Countries.FirstOrDefault(i => i.Name.ToLower() == leadsEditViewModel.FinalTransferedOffer.Site.Country.Name.ToLower()).Iso + ".png";
                                    dataRow["Region"] = leadsEditViewModel.FinalTransferedOffer.Site.Country.Zone != null ? leadsEditViewModel.FinalTransferedOffer.Site.Country.Zone.Name : "";
                                }
                            ...
                            dataRow["Description"] = leadsEditViewModel.FinalTransferedOffer.Description;

                            if (leadsEditViewModel.FinalTransferedOffer.GeosStatus != null)
                            {
                                dataRow["Status"] = leadsEditViewModel.FinalTransferedOffer.GeosStatus.Name;
                                dataRow["HtmlColor"] = leadsEditViewModel.FinalTransferedOffer.GeosStatus.HtmlColor;
                                // dataRow["IdOfferStatusType"] = leadsEditViewModel.OfferData.GeosStatus.IdOfferStatusType;
                            }

                            dataRow["Amount"] = leadsEditViewModel.FinalTransferedOffer.OfferValue;
                            dataRow["Discount"] = leadsEditViewModel.FinalTransferedOffer.Discount;//[rdixit][06.07.2022]
                            dataRow["OfferCloseDate"] = leadsEditViewModel.FinalTransferedOffer.DeliveryDate;
                            dataRow["IsCloseDateExceed"] = false;
                            dataRow["OfferConfidentialLevel"] = leadsEditViewModel.FinalTransferedOffer.ProbabilityOfSuccess.ToString();

                            dataRow["SalesOwner"] = leadsEditViewModel.FinalTransferedOffer.SalesOwner != null ? leadsEditViewModel.FinalTransferedOffer.SalesOwner.FullName : "";
                            ...
                            {
                                User user = leadsEditViewModel.OfferOwnerList.Where(x => x.IdUser == leadsEditViewModel.FinalTransferedOffer.OfferedBy).FirstOrDefault();
                            ...
                            if (leadsEditViewModel.FinalTransferedOffer.CarProject != null)
                                dataRow["Project"] = leadsEditViewModel.FinalTransferedOffer.CarProject.Name;
                            if (leadsEditViewModel.FinalTransferedOffer.CarOEM != null)
                                dataRow["OEM"] = leadsEditViewModel.FinalTransferedOffer.CarOEM.Name;
                            if (leadsEditViewModel.FinalTransferedOffer.Source != null)
                                dataRow["Source"] = leadsEditViewModel.FinalTransferedOffer.Source.Value;
                            if (leadsEditViewModel.FinalTransferedOffer.BusinessUnit != null)
                                dataRow["BusinessUnit"] = leadsEditViewModel.FinalTransferedOffer.BusinessUnit.Value;

                            if (leadsEditViewModel.FinalTransferedOffer.RFQReception == null)
                            {
                                dataRow["RFQReceptionDate"] = DBNull.Value;
                            }
                            else if (leadsEditViewModel.FinalTransferedOffer.RFQReception != null && leadsEditViewModel.FinalTransferedOffer.RFQReception == DateTime.MinValue)
                                dataRow["RFQReceptionDate"] = DBNull.Value;
                            else
                                dataRow["RFQReceptionDate"] = leadsEditViewModel.FinalTransferedOffer.RFQReception.Value;

                            if (leadsEditViewModel.FinalTransferedOffer.Rfq == null)
                            {
                                dataRow["Rfq"] = DBNull.Value;
                            }
                            else
                            {
                                dataRow["Rfq"] = leadsEditViewModel.FinalTransferedOffer.Rfq;
                            }
                            if (leadsEditViewModel.FinalTransferedOffer.SendIn == null)
                            {
                                dataRow["QuoteSentDate"] = DBNull.Value;
                            }
                            else if (leadsEditViewModel.FinalTransferedOffer.SendIn != null && leadsEditViewModel.FinalTransferedOffer.SendIn == DateTime.MinValue)
                                dataRow["QuoteSentDate"] = DBNull.Value;
                            else
                                dataRow["QuoteSentDate"] = leadsEditViewModel.FinalTransferedOffer.SendIn.Value;
                            if (leadsEditViewModel.FinalTransferedOffer.GeosStatus.IdOfferStatusType == 1)
                            {
                                if (leadsEditViewModel.FinalTransferedOffer.LastActivityDate == null)
                                {
                                ...
                                {
                                    dataRow["SleepDays"] = Convert.ToInt32((GeosApplication.Instance.ServerDateTime.Date - leadsEditViewModel.FinalTransferedOffer.LastActivityDate.Value.Date).TotalDays);

                                }
                            }

                            foreach (var item in leadsEditViewModel.FinalTransferedOffer.OptionsByOffers)
                            {
                            ...
                            //[CRM-M040-04]
                            if (leadsEditViewModel.FinalTransferedOffer.LostReasonsByOffer != null)
                            {
                                if (leadsEditViewModel.FinalTransferedOffer.LostReasonsByOffer.Competitor != null)
                                    dataRow["LostCompetitor"] = leadsEditViewModel.FinalTransferedOffer.LostReasonsByOffer.Competitor.Name;

                                if (leadsEditViewModel.FinalTransferedOffer.LostReasonsByOffer.OfferLostReason != null)
                                    dataRow["LostReason"] = leadsEditViewModel.FinalTransferedOffer.LostReasonsByOffer.OfferLostReason.Name;

                                dataRow["LostDescription"] = leadsEditViewModel.FinalTransferedOffer.LostReasonsByOffer.Comments;
                            }

                            if (leadsEditViewModel.IsCheckedDuringTransferOffer != true) // for remove the offer if IsChecked = false
                            {
                                DataRow dataRow1 = Dttable.AsEnumerable().FirstOrDefault(row => Convert.ToInt64(row["Idoffer"]) == leadsEditViewModel.SelectedOfferIdOffer);
                                Dttable.Rows.Remove(dataRow1);
                            }
                        ...
                        #region Else for FinalTransfered offer != null
                        else
                        {
                            ...
                            if (idCategory > 0)
                            {
                                ...
                                if (objProCat.MergedCategoryAndProduct != null)
                                {
                                    ...
                                    if (strArray.Count() < 2)
                                    {
                                        dataRow["Category1"] = strArray[0] != null ? strArray[0].Trim() : strArray[0];
                                        dataRow["Category2"] = null;
                                    }
                                    else
                                    {
                                        dataRow["Category1"] = strArray[0] != null ? strArray[0].Trim() : strArray[0];
                                        dataRow["Category2"] = strArray[1] != null ? strArray[1].Trim() : strArray[1];
                                    }
                                }
                            }
                            ...
                            if (leadsEditViewModel.OfferData.Site != null)
                            {
                                ...
                                if (leadsEditViewModel.OfferData.Site.Country != null)
                                {
                                    ...
                                    //[GEOS2-4323][17.04.2023][rdixit]
                                    dataRow["CountryImageUrl"] = "https://api.emdep.com/GEOS/Images?FilePath=/Images/Countries/" + Countries.FirstOrDefault(i => i.Name.ToLower() == leadsEditViewModel.OfferData.Site.Country.Name.ToLower()).Iso + ".png";
                            ...
                            dataRow["Discount"] = leadsEditViewModel.OfferData.Discount;//[rdixit][06.07.2022]
                        ...
                        foreach (var OfferDataItem in leadsEditViewModel.OfferDataLst)
                        {
                            ...
                            if (OfferDataItem.Site != null)
                            {
                                ...
                                if (OfferDataItem.Site.Country != null)
                                {
                                    ...
                                    //[GEOS2-4323][17.04.2023][rdixit]
                                    dataRow["CountryImageUrl"] = "https://api.emdep.com/GEOS/Images?FilePath=/Images/Countries/" + Countries.FirstOrDefault(i => i.Name.ToLower() == OfferDataItem.Site.Country.Name.ToLower()).Iso + ".png";//;
                            ...
                            dataRow["Discount"] = OfferDataItem.Discount;//[rdixit]
                            #region Comments
                            //Fill List For Different Sales Owner
                            //[rdixit][GEOS2-4284][10.05.2023] Updated service GetSalesOwnerBySiteId by GetSalesOwnerBySiteId_V2390
                            //[rdixit][GEOS2-4903][26.10.2023] Service GetSalesOwnerBySiteId_V2390 updated with GetSalesOwnerBySiteId_V2450 
                            #endregion
                            var SalesOwners = CrmStartUp.GetSalesOwnerBySiteId_V2450(OfferDataItem.Site.IdCompany);
                            ...
                            OfferDataItem.BusinessUnit = (LookupValue)BusinessUnitList.Where(item3 => item3.IdLookupValue == OfferDataItem.IdBusinessUnitNew).FirstOrDefault();
                            OfferDataItem.Source = (LookupValue)LeadSourceList.Where(item4 => item4.IdLookupValue == OfferDataItem.IdSourceNew).FirstOrDefault();
        ...
        private void LeadsNewViewWindowShow(object obj)
        {
            try
            {
                ...
                leadsAddView.Owner = Window.GetWindow(ownerInfo);
                ...
                if (leadAddViewModel.OfferData != null)
                {
                    ...
                    if (leadAddViewModel.OfferData.Site != null)
                    {
                        ...
                        //dataRow["Site"] = GeosApplication.Instance.CompanyList.Where(cmplst => cmplst.ConnectPlantId == dataRow["ConnectPlantId"].ToString()).Select(slt => slt.Alias).FirstOrDefault();
                        dataRow["Site"] = leadAddViewModel.OfferData.Site.ShortName;
                        ...
                        if (leadAddViewModel.OfferData.Site.Country != null)
                        {
                            ...
                            //[GEOS2-4323][17.04.2023][rdixit]
                            dataRow["CountryImageUrl"] = "https://api.emdep.com/GEOS/Images?FilePath=/Images/Countries/" + Countries.FirstOrDefault(i => i.Name.ToLower() == leadAddViewModel.OfferData.Site.Country.Name.ToLower()).Iso + ".png";//
                    ...
                    dataRow["Discount"] = leadAddViewModel.OfferData.Discount;//[rdixit][GEOS2-2909][06.07.2022]
                    ...
                    //[rdixit][GEOS2-4903][26.10.2023] Service GetSalesOwnerBySiteId_V2390 updated with GetSalesOwnerBySiteId_V2450 
                    #endregion
                    var SalesOwners = CrmStartUp.GetSalesOwnerBySiteId_V2450(leadAddViewModel.OfferData.Site.IdCompany);
        ...
        //[nsatpute][18.07.2025][GEOS2-8840]
        private void CustomCellAppearanceGridControl(RoutedEventArgs obj)
        {
            try
            {
                ...
                GeosApplication.Instance.CustomeSplashScreenInformationMessage = "Restoring grid layout...";
		...
		/// [002][GEOS2-7205][nsatpute][06-06-2025]
		/// [003][nsatpute][18.07.2025][GEOS2-8840]
        /// </summary>
                        ...
                        if (plantOwners.Count == 1)
                        {
                            foreach (var itemPlantOwnerUsers in plantOwners)
                            {
                                try
                                {
                                    ...
                                    objTimelineParams.accountingYearTo = Convert.ToDateTime(DateTime.MaxValue.ToString("yyyy-MM-dd"));//[rdixit][29.06.2022][GEOS2-3513][Ignore the “To” date in the “OPPORTUNITIES-Timeline” section]
                        ...
                        else
                        {
                            Parallel.ForEach(plantOwners, itemPlantOwnerUsers =>
                            {
                                try
                                {
                                    ...
                                    objTimelineParams.activeSite = new ActiveSite { IdSite = Convert.ToInt32(itemPlantOwnerUsers.ConnectPlantId), SiteAlias = Convert.ToString(itemPlantOwnerUsers.Alias), SiteServiceProvider = itemPlantOwnerUsers.ServiceProviderUrl };
                                    objTimelineParams.accountingYearFrom = GeosApplication.Instance.SelectedyearStarDate;
                                    objTimelineParams.accountingYearTo = Convert.ToDateTime(DateTime.MaxValue.ToString("yyyy-MM-dd"));
                                    objTimelineParams.Roles = RoleType.SalesGlobalManager;

                                    timelineGridListtemp = GetTimelineGridData(objTimelineParams).ToList();

                                    lock (TimelineGridList)
                                    {
                                        TimelineGridList.AddRange(timelineGridListtemp);
                                    }
                ...
                if (GeosApplication.Instance.IdUserPermission == 21)
                {
                    ...
                    else
                    {
                        Parallel.ForEach(GeosApplication.Instance.CompanyList, itemCompaniesDetails =>
                        {
                            try
                            {
                                var objTimelineParams = new TimelineParams
                                {
                                    idCurrency = GeosApplication.Instance.IdCurrencyByRegion,
                                    idsSelectedUser = salesOwnersIds,
                                    idUser = GeosApplication.Instance.ActiveUser.IdUser,
                                    idZone = GeosApplication.Instance.ActiveUser.Company.Country.IdZone,
                                    activeSite = new ActiveSite
                                    {
                                        IdSite = Convert.ToInt32(itemCompaniesDetails.ConnectPlantId),
                                        SiteAlias = itemCompaniesDetails.Alias.ToString(),
                                        SiteServiceProvider = itemCompaniesDetails.ServiceProviderUrl
                                    },
                                    accountingYearFrom = GeosApplication.Instance.SelectedyearStarDate,
                                    accountingYearTo = Convert.ToDateTime(DateTime.MaxValue.ToString("yyyy-MM-dd")),
                                    Roles = RoleType.SalesPlantManager
                                };

                                var timelineGridListtemp = GetTimelineGridData(objTimelineParams).ToList();
        ...
        private ObservableCollection<TimelineGrid> GetTimelineGridData(TimelineParams objTimelineParams)
        {
            ...
            //objTimelineParams.activeSite.SiteServiceProvider = "localhost:6699";
            //[GEOS2-4323][13.04.2023][rdixit] Service Method Updated from GetTimelineGridDetails_V2200to GetTimelineGridDetails_V2380
            //[GEOS2-4284][10.05.2023][rdixit] Service Method Updated from GetTimelineGridDetails_V2380 by GetTimelineGridDetails_V2390
            //[rdixit][GEOS2-4712][01.08.2023] Service Method Updated from GetTimelineGridDetails_V2390 by GetTimelineGridDetails_V2420
            #endregion
            string ServiceUrl = null;
            if (objTimelineParams.activeSite.SiteServiceProvider.Contains("http://") || objTimelineParams.activeSite.SiteServiceProvider.Contains("https://"))
            {
                // [003] changed Service Method
                ServiceUrl = objTimelineParams.activeSite.SiteServiceProvider + "/CrmRestService.svc" + "/GetTimelineGridDetails_V2420";
                ...
                ServiceUrl = "http://" + objTimelineParams.activeSite.SiteServiceProvider + "/CrmRestService.svc" + "/GetTimelineGridDetails_V2420";

            //  ServiceUrl = "http://" + "localhost:6699" + "/CrmRestService.svc" + "/GetTimelineGridDetails_V2420";
        ...
        private void FillLeads()
        {
            try
            {
                ...
                var timelineGridConcurrentBag = new ConcurrentBag<TimelineGrid>();
                ...
                if (GeosApplication.Instance.CompanyList.Count == 1)
                {
                    foreach (var itemCompaniesDetails in GeosApplication.Instance.CompanyList)
                    {
                        try
                        {
                            ...
                            objTimelineParams.accountingYearFrom = GeosApplication.Instance.SelectedyearStarDate;
                            //objTimelineParams.accountingYearTo = GeosApplication.Instance.SelectedyearEndDate;
                            //Shubham[skadam] GEOS2-4094 CRM data wrong showing 21 12 2022
                            objTimelineParams.accountingYearTo = Convert.ToDateTime(DateTime.MaxValue.ToString("yyyy-MM-dd"));//[rdixit][29.06.2022][GEOS2-3513][Ignore the “To” date in the “OPPORTUNITIES-Timeline” section]
                ...
                else
                {
                    Parallel.ForEach(GeosApplication.Instance.CompanyList, itemCompaniesDetails =>
                    {
                        ...
                        try
                        {
                            ...
                            objTimelineParams.idsSelectedUser = GeosApplication.Instance.ActiveUser.IdUser.ToString();
                            objTimelineParams.idZone = GeosApplication.Instance.ActiveUser.Company.Country.IdZone;
                            objTimelineParams.activeSite = new ActiveSite
                            {
                                IdSite = Convert.ToInt32(itemCompaniesDetails.ConnectPlantId),
                                SiteAlias = Convert.ToString(itemCompaniesDetails.Alias),
                                SiteServiceProvider = itemCompaniesDetails.ServiceProviderUrl
                            };
                            objTimelineParams.accountingYearFrom = GeosApplication.Instance.SelectedyearStarDate;
                            objTimelineParams.accountingYearTo = Convert.ToDateTime(DateTime.MaxValue.ToString("yyyy-MM-dd"));
        ...
        private void AddDataTableColumns()
        {
            try
            {
                ...
                Columns = new ObservableCollection<Emdep.Geos.UI.Helper.Column>() {
                        ...
                        new Emdep.Geos.UI.Helper.Column() {FieldName="IdSite",HeaderText="IdSite", Settings = SettingsType.IsChecked, AllowCellMerge=false, Width=120,AllowEditing=false,Visible=false,IsVertical= false,FixedWidth=false  },
                        ...
                        //new Emdep.Geos.UI.Helper.Column() { FieldName="Project",HeaderText="Project", Settings = SettingsType.Default, AllowCellMerge=false, Width=120,AllowEditing=true,Visible=true,IsVertical= false,FixedWidth=false   },
                        ...
                        new Emdep.Geos.UI.Helper.Column() { FieldName="Country",HeaderText="Country", Settings = SettingsType.CountryWithImage, AllowCellMerge=false,Width=110,AllowEditing=false,Visible=true,IsVertical= false,FixedWidth=false },
                        ...
                        new Emdep.Geos.UI.Helper.Column() { FieldName="Status", HeaderText="Status",Settings = SettingsType.Status, AllowCellMerge=false,Width=50,AllowEditing=false,Visible=true,IsVertical= false,FixedWidth=false  },
                        new Emdep.Geos.UI.Helper.Column() { FieldName="HtmlColor", HeaderText="HtmlColor",Settings = SettingsType.Hidden, AllowCellMerge=false,Width=80,AllowEditing=false,Visible=false,IsVertical= false,FixedWidth=false  },
                        new Emdep.Geos.UI.Helper.Column() { FieldName="Amount", HeaderText="Amount",Settings = SettingsType.Amount, AllowCellMerge=false,Width=170,AllowEditing=true,Visible=false,IsVertical= false,FixedWidth=false  },
                        new Emdep.Geos.UI.Helper.Column() { FieldName="Discount", HeaderText="Discount",Settings = SettingsType.Discount, AllowCellMerge=false,Width=70,AllowEditing=true,Visible=false,IsVertical= false},
                        new Emdep.Geos.UI.Helper.Column() { FieldName="OfferCloseDate", HeaderText="CloseDate",Settings = SettingsType.CloseDate, AllowCellMerge=false,Width=90,AllowEditing=true,Visible=true,IsVertical= false,FixedWidth=false  },
                        new Emdep.Geos.UI.Helper.Column() { FieldName="OfferStatusid", HeaderText="OfferStatusid",Settings = SettingsType.Hidden, AllowCellMerge=false,Width=90,AllowEditing=true,Visible=false   ,IsVertical= false,FixedWidth=false  },


                        new Emdep.Geos.UI.Helper.Column() { FieldName="OfferConfidentialLevel", HeaderText="Confidence Level",Settings = SettingsType.PercentText, AllowCellMerge=false,Width=80,AllowEditing=true,Visible=true,IsVertical= false,FixedWidth=false  },
                        ...
                        new Emdep.Geos.UI.Helper.Column() { FieldName="IsCloseDateExceed", HeaderText="IsCloseDateExceed",Settings = SettingsType.Hidden, AllowCellMerge=false,Width=80,AllowEditing=false,Visible=false,IsVertical= false,FixedWidth=false  },
                        ...
                        new Emdep.Geos.UI.Helper.Column() { FieldName="OEM",HeaderText="OEM", Settings = SettingsType.OEM, AllowCellMerge=false, Width=120,AllowEditing=true,Visible=false,IsVertical= false,FixedWidth=false  },
                        new Emdep.Geos.UI.Helper.Column() { FieldName="Source",HeaderText="Source", Settings = SettingsType.Source, AllowCellMerge=false, Width=120,AllowEditing=true,Visible=false,IsVertical= false,FixedWidth=false   },
                        new Emdep.Geos.UI.Helper.Column() { FieldName="BusinessUnit",HeaderText="Business Unit", Settings = SettingsType.BusinessUnit, AllowCellMerge=false, Width=120,AllowEditing=true,Visible=false,IsVertical= false,FixedWidth=false   },
                        new Emdep.Geos.UI.Helper.Column() { FieldName="RFQReceptionDate", HeaderText="RFQ Reception Date",Settings = SettingsType.RFQDate, AllowCellMerge=false,Width=90,AllowEditing=false,Visible=false,IsVertical= false,FixedWidth=false  },
                        new Emdep.Geos.UI.Helper.Column() { FieldName="QuoteSentDate", HeaderText="Quote Sent Date",Settings = SettingsType.QuoteSentDate, AllowCellMerge=false,Width=90,AllowEditing=false,Visible=false,IsVertical= false,FixedWidth=false  },
                        new Emdep.Geos.UI.Helper.Column() { FieldName="Rfq", HeaderText="RFQ",Settings = SettingsType.RFQ, AllowCellMerge=false,Width=90,AllowEditing=false,Visible=false,IsVertical= false,FixedWidth=false  },
                        ...
                        //[001] added
                        new Emdep.Geos.UI.Helper.Column() { FieldName="OfferOwner",HeaderText="Offer Owner", Settings = SettingsType.OfferOwner, AllowCellMerge=false, Width=70,AllowEditing=true,Visible=false,IsVertical= false,FixedWidth=false   },
                        new Emdep.Geos.UI.Helper.Column() { FieldName="OfferedTo",HeaderText="Offered To", Settings = SettingsType.OfferedTo, AllowCellMerge=false, Width=70,AllowEditing=true,Visible=false,IsVertical= false,FixedWidth=false   },
                        new Emdep.Geos.UI.Helper.Column() { FieldName="ActiveSite",HeaderText="ActiveSite", Settings = SettingsType.Hidden, AllowCellMerge=false, Width=70,AllowEditing=true,Visible=false,IsVertical= false,FixedWidth=false   },
                ...
                Columns.Add(new Emdep.Geos.UI.Helper.Column() { FieldName = "CountryImageUrl", HeaderText = "CountryImageUrl", Settings = SettingsType.Hidden, AllowCellMerge = false, Width = 10, AllowEditing = false, Visible = false, IsVertical = false, FixedWidth = true });//[GEOS2-4323][13.04.2023][rdixit]
                Dttable.Columns.Add("CountryImageUrl", typeof(string)); //[GEOS2-4323][13.04.2023][rdixit]
        ...
        private void FillDataTable()
        {
            ...
            for (int i = 0; i < TimelineGridList.Count; i++)
            {
                try
                {
                    ...
                    dr["CountryImageUrl"] = "https://api.emdep.com/GEOS/Images?FilePath=/Images/Countries/" + TimelineGridList[i].Iso + ".png"; //[GEOS2-4323][13.04.2023][rdixit]
                    ...
                    if (TimelineGridList[i].DiscountNullable != null)
                        dr["Discount"] = TimelineGridList[i].DiscountNullable;
                    else
                    ...
                    #region Previous SalesOwnerCode Commented
                    /*
                    ...
                    */
                    ...
                    #region [GEOS2-4284][rdixit][10.05.2023]
                    if (!(GeosApplication.Instance.SalesOwnerList.Any(sw => sw.IdSite == Convert.ToInt32(TimelineGridList[i].IdSite))))
                    {
                        if (TimelineGridList[i].SalesOwnerList != null)
                        {
                            foreach (var item in TimelineGridList[i].SalesOwnerList)
                            {
                                SalesOwnerList _salesOwner = new SalesOwnerList();
                                _salesOwner.IdOffer = TimelineGridList[i].IdOffer;
                                _salesOwner.IdSalesOwner = item.IdSalesOwner;
                                _salesOwner.SalesOwner = item.SalesOwner;
                                if (item.IsSiteResponsibleExist == true)
                                    _salesOwner.IsSiteResponsibleExist = true;
                                else
                                    _salesOwner.IsSiteResponsibleExist = false;
                                _salesOwner.IdSite = Convert.ToInt32(TimelineGridList[i].IdSite);
                    ...
                    if (TimelineGridList[i].IdSalesOwner != null)
                    {
                        dr["SalesOwner"] = GeosApplication.Instance.PeopleList.Where(x => x.IdPerson == TimelineGridList[i].IdSalesOwner).Select(slt => slt.FullName).FirstOrDefault();
                        dr["IsSaleOwnerNull"] = false;
                    }
                    else if (TimelineGridList[i].SalesOwnerList != null)
                    {
                        if (TimelineGridList[i].SalesOwnerList.Count == 1)
                        {
                            dr["SalesOwner"] = TimelineGridList[i].SalesOwnerList.FirstOrDefault().SalesOwner;
                            dr["IsSaleOwnerNull"] = true;
                        }
                        else
                        {
        ...
        public void FillFilterTileBar()
        {
            try
            {
                ...
                if (Dttable != null)
                {
                    ...
                    List<int> idOfferStatusTypeList = DttableList.Select(x => (int)x.ItemArray[34]).Distinct().ToList();
                    ...
                    foreach (var item in tempStatusList)
                    {
                        FilterStatusListOfTile.Add(new TileBarFilters()
                        {
                            ...
                            EntitiesCount = (DttableList.Where(x => (int)x.ItemArray[34] == item.IdOfferStatusType).ToList()).Count,
                            EntitiesCountVisibility = Visibility.Visible,
                            Height = 80,
                            width = 200
                        });
        ...
        private void CommandFilterStatusTileClickAction(object obj)
        {
            try
            {
                ...
                if (test?.Count() > 0)
                {
                    ...
                    string FilterString = ((TileBarFilters)test[0]).FilterCriteria;
        ...
        /// <summary>
        //[004][soumitra.kulkarni][16/05/2025][GEOS-6583]CRM Opportunity Timeline conditional formating filter issue.
        // Method to restore conditional formatting settings
        /// </summary>
        #region GridStatePersistence
        private void SaveFullGridState()
        {
            try
            {
                if (GridControl == null || view == null)
                {
                    GeosApplication.Instance.Logger.Log(
                        "GridControl or view is null. Exiting SaveFullGridState.",
                ...
                {
                    GridControl.SaveLayoutToStream(ms);
                    File.WriteAllBytes(path, ms.ToArray());
                }
                GeosApplication.Instance.Logger.Log(
                    "Full grid state saved to LeadsGridState.bin",
                    Category.Info, Priority.Low);
            }
            catch (Exception ex)
            {
                GeosApplication.Instance.Logger.Log(
                    "Error in SaveFullGridState: " + ex.Message,
                    Category.Exception, Priority.Low);
            }
        }
        //[nsatpute][19.06.2025][GEOS2-6689]
        private async Task RestoreFullGridStateAsync()
        {
            ...
            try
            {
                ...
                using (var fs = new FileStream(
                    ...
                    FileShare.Read,
                    bufferSize: 4096,
                    useAsync: true)) // Async I/O in .NET Framework
                {
                    data = new byte[fs.Length];
                    await fs.ReadAsync(data, 0, (int)fs.Length).ConfigureAwait(false);
                }

                grid.RestoreLayoutFromStream(new MemoryStream(data));
            }
            catch (FileNotFoundException) { }
            catch (Exception ex)
            {
                GeosApplication.Instance.Logger.Log(
                    $"Error in RestoreFullGridState: {ex.Message}",
                    Category.Exception, Priority.Low);
            }
            if (DXSplashScreen.IsActive) { DXSplashScreen.Close(); }
        }
        public void InitializeGridControl(GridControl gridControl)
        {
            GridControl = gridControl;
            view = gridControl.View as TableView;
            if (view == null) return;


            view.FormatConditions.CollectionChanged += OnFormatConditionsChanged;

            //[nsatpute][19.06.2025][GEOS2-6689]
            RestoreFullGridStateAsync();
        }
        private void OnFormatConditionsChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            if (_suspendAutoSave) return;
            SaveFullGridState();
        }
        private void ConditionalFormattingChanged(object obj)
        {
            SaveFullGridState();
        }
        #endregion
