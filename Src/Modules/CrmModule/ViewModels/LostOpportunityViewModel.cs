...
namespace Emdep.Geos.Modules.Crm.ViewModels
{
    class LostOpportunityViewModel : NavigationViewModelBase, IDataErrorInfo, INotifyPropertyChanged
    {
        ...
        public LostReasonsByOffer LostReasonsByOfferForRetrive;
        ...
        public void InIt()
        {
            try
            {
                ...
                if (offer!=null && offer[0].LostReasonsByOffer != null)
                {
                ...
                else if(LostReasonsByOfferForRetrive!=null)
                {
                    if (!string.IsNullOrEmpty(LostReasonsByOfferForRetrive.Comments))
                    {
                        LostReasonDescription = LostReasonsByOfferForRetrive.Comments;
                    }

                    if (LostReasonsByOfferForRetrive.IdLostReasonList != null)
                    {
                        List<string> selectedReason = new List<string>(LostReasonsByOfferForRetrive.IdLostReasonList.Split(';'));
                        SelectedItems = new List<object>();
                        for (int i = 0; i < selectedReason.Count; i++)
                        {
                            SelectedItems.Add(OfferLostReasons.Where(n => n.IdLostReason == int.Parse(selectedReason[i].ToString())).SingleOrDefault());
                    ...
                    SelectedIndexCompetitors = Competitors.FindIndex(i => i.IdCompetitor == LostReasonsByOfferForRetrive.IdCompetitor);

                    //[001] added
                    if (LostReasonsByOfferForRetrive.IdOffer > 0)
                        OfferLostDate = OfferCloseDate;
                    else
                        OfferLostDate = GeosApplication.Instance.ServerDateTime.Date;
                }
