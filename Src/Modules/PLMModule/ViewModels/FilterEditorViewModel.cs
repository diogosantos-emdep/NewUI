    ...
    public class FilterEditorViewModel : INotifyPropertyChanged, IDataErrorInfo
    {
        ...
        private void FilterEditorViewAcceptCommandAction(object obj)
        {
            try
            {
                ...
                {
                    FilterStatusList = new ObservableCollection<TileBarFilters>();
                }
                var existFilter = FilterStatusList.FirstOrDefault(x => x.Caption.Equals(FilterName, StringComparison.InvariantCultureIgnoreCase));
                if (IsNew)
                {
                    ...
                    if (existFilter == null)
                    {
                        ...
                        if (filterControl.FilterCriteria!=null)
                        {
                            FilterCriteria = filterControl.FilterCriteria.ToString();
                        }
                        else
                        {
                            FilterCriteria = filterControl.ActualFilterCriteria.ToString();
                        }
                ...
                {
                    if (existFilter == null || ExistFilterName.Equals(FilterName, StringComparison.InvariantCultureIgnoreCase))
                    {
                        filterControl.ApplyFilter();
                        FilterCriteria = filterControl.FilterCriteria.ToString();
                        IsDelete = false;
