...
{
    public static class ERMCamCadTimeTrackingGetData
    {

        public static CancellationTokenSource tokenToLoadFullCamCadTimetrackingInAsync;
        public static System.Threading.Tasks.Task<List<TimeTracking>> taskToLoadCamCadTimetrackingInAsync;
        ...
        public static async void GetERMCamCadTimeTrackingFromServiceAsync(
IERMService ERMService, IPLMService PLMService, List<TimeTracking> TimeTrackingListold,
 GridControl GridControl1, string CurrencyNameFromSetting, string PlantName, DataTable DtTimetracking, DataTable DataTableForGridLayout
 , List<GeosAppSetting> GeosAppSettingList, List<ERMTimetrackingSite> PlantList, ObservableCollection<TrackingAccordian> TemplateWithTimeTracking, string WarningFailedPlants, bool IsShowFailedPlantWarning, List<Site> AllPlantsList, List<TimeTrackingProductionStage> TimeTrackingProductionStage, StageByOTItemAndIDDrawing TimetrackingStagesList, List<ERM_CADCAMTimePerDesignType> CADCAMDesignTypeList, List<int> AppSettingData_old, string fromDate, string toDate)

        {
            const string methodNameWithBrackets = nameof(GetERMCamCadTimeTrackingFromServiceAsync) + "()";
            try
            {
                ...
                foreach (string itemSelectedPlant in PlantName.Split(','))
                {
                    ...
                    int count = 1;
                    //foreach (var timeTrackingStage in PlantList.Where(i => i.Name != itemSelectedPlant.Trim()))  ////[GEOS2-5098] [gulab lakade][30 11 2023]
                    foreach (var item in PlantList.Where(i => i.ProductionSite != itemSelectedPlant.Trim()))  ////[GEOS2-5098] [gulab lakade][30 11 2023]
                    {
                        ...
                        if (ERMCommon.Instance.UnloadAsyncTimetracking == true)
                        {
                            ...
                            //ERMCommon.Instance.TimeTrackingLoadingMessage = "Please wait while getting data from the plant " + timeTrackingStage.Name + "  in background...";
                            //string serviceurl = GeosApplication.Instance.GeosServiceProviders.GeosServiceProvider.Where(x => x.Name == timeTrackingStage.Name).Select(xy => xy.ServiceProviderUrl).FirstOrDefault();
                            ...
                            if (serviceurl != null)
                            {
                                ...
                                if (tokenToLoadFullCamCadTimetrackingInAsync != null
                                    //&& taskToLoadFullYearAttendanceInAsync.Status==TaskStatus.
                                    )
                                {
                                    try
                                    {
                                        tokenToLoadFullCamCadTimetrackingInAsync.Cancel();
                                        taskToLoadCamCadTimetrackingInAsync.Wait(tokenToLoadFullCamCadTimetrackingInAsync.Token);
                                ...
                                tokenToLoadFullCamCadTimetrackingInAsync = new CancellationTokenSource();
                                ...
                                //ERMService, PLMService, itemSelectedPlant, PlantList.Where(i => i.Name == itemSelectedPlant.Trim()).FirstOrDefault().IdSite, timeTrackingStage.Name, GeosAppSettingList, TimeTrackingListold);

                                //var newTimeTracking = await GetERMTimeTrackingFromServiceAsyncTask(
                                //ERMService, PLMService, itemSelectedPlant, AllPlantsList.Where(i => i.Name == itemSelectedPlant.Trim()).FirstOrDefault().IdSite, timeTrackingStage.Name, GeosAppSettingList, TimeTrackingListold);////[GEOS2-5098] [gulab lakade][30 11 2023]
                                var newTimeTracking = await GetERMCamCadTimeTrackingFromServiceAsyncTask(
                               ERMService, PLMService, itemSelectedPlant, AllPlantsList.Where(i => i.Name == itemSelectedPlant.Trim()).FirstOrDefault().IdSite, item, GeosAppSettingList, TimeTrackingListold, fromDate, toDate);  ////[GEOS2-5098] [gulab lakade][30 11 2023]
                                ...
                                if (tokenToLoadFullCamCadTimetrackingInAsync != null &&
                                    tokenToLoadFullCamCadTimetrackingInAsync.IsCancellationRequested)
                                {
                                    return;
                                }
                        ...
                        {
                            tokenToLoadFullCamCadTimetrackingInAsync.Cancel();
                            tokenToLoadFullCamCadTimetrackingInAsync.Dispose();
                            tokenToLoadFullCamCadTimetrackingInAsync.Token.ThrowIfCancellationRequested();
        ...
        public static async Task<List<TimeTracking>>
    GetERMCamCadTimeTrackingFromServiceAsyncTask(
        ...
        {
            const string methodNameWithBrackets = nameof(GetERMCamCadTimeTrackingFromServiceAsyncTask) + "()";
            var TimeTrackingList = new List<TimeTracking>();
            try
            {
                GeosApplication.Instance.Logger.Log($"Method {methodNameWithBrackets}...", category: Category.Info, priority: Priority.Low);
                ...
                ERMCommon.Instance.IsVisibleLabelTimetrackingInBackground = Visibility.Visible;
                taskToLoadCamCadTimetrackingInAsync =
                    new Task<List<TimeTracking>>(() =>
                    {
                    ...
                    }, tokenToLoadFullCamCadTimetrackingInAsync.Token);
                taskToLoadCamCadTimetrackingInAsync.Start();
                GeosApplication.Instance.Logger.Log($"Method {methodNameWithBrackets}....executed successfully", category: Category.Info, priority: Priority.Low);
                return await taskToLoadCamCadTimetrackingInAsync;
            }
            catch (FaultException<ServiceException> ex)
            {
...

        {
            try
            {
                ...
                for (int i = 0; i < TimeTrackingListnew.Count; i++)
                {
                    ...
                    if (TimeTrackingListnew[i].DesignSystem != null)
                    {
                        dr["DesignSystem"] = Convert.ToString(TimeTrackingListnew[i].DesignSystem);
                    }

                    var OperatorNames = string.Join(",", TimeTrackingListnew[i].TimeTrackingAddingPostServer.Where(a => a.TimeTrackIdCounterpart == TimeTrackingListnew[i].IdCounterpart).Select(x => x.OperatorName.Trim()).Distinct(StringComparer.OrdinalIgnoreCase).ToList());
                    ...
                    dr["StartRev"] = Convert.ToString(TimeTrackingListnew[i].StartRevision);

                    dr["LastRev"] = Convert.ToString(TimeTrackingListnew[i].LastRevision);
                    ...
                    if (TimeTrackingListnew[i].TimeTrackingStage != null)
                    {
                        ...
                        try
                        {
                            ...
                            foreach (var timeTrackingStage in reorderedResult1)
                            {
                                ...
                                var Stagerecord = TimeTrackingProductionStage.Where(x => x.IdStage == timeTrackingStage.IdStage).FirstOrDefault();
                                if (Stagerecord != null)
                                {
                                    ...
                                    if (FlagPresentInActivePlants == true || Stagerecord.ActiveInPlants == null || Stagerecord.ActiveInPlants == "")
                                    {
                                        if (TempTimeTrackingByStageList.Count() > 0)
                                        {
                                            ...
                                            {
                                                TimeTrackingCurrentStage TempTimeTrackingByStage = new TimeTrackingCurrentStage();
                                                //var tempexpexcted = temprecord.Where(X => X.IdStage == timeTrackingStage.IdStage && X.Expected!=null).FirstOrDefault();
                                        ...
                                        {
                                            TimeTrackingCurrentStage TempTimeTrackingByStage = new TimeTrackingCurrentStage();
                                            // var tempexpexcted = temprecord.Where(X => X.IdStage == timeTrackingStage.IdStage).FirstOrDefault();
                                                ...
                                                if (checkIdstage_first == timeTrackingStage.IdStage)
                                                {
                                                    try
                                                    {
                                                        ...
                                                        if (DataTableForGridLayout.Columns.Contains(production_first))
                                                        {
                                                            if (Convert.ToString(dr[production_first]) != null)
                                                            {
                                                                ...
                                                                if (DataTableForGridLayout.Columns.Contains(production_first))
                                                                {
                                                                    ...
                                                                    else if (timeTrackingStage.IdStage == 2 && TimeTrackingListnew[i].DesignSystem == "GSM")
                                                                    {
                                                                        #region [GEOS2-7094][dhawal bhalerao][15-05-2025] 

                                                                        var tempAddingPosServer = TimeTrackingListnew[i].TimeTrackingAddingPostServer.Where(x => x.TimeTrackIdCounterparttracking == timeTrackingStage.IdCounterparttracking && (x.ProductionActivityTimeType == 0 || x.ProductionActivityTimeType == 1920 || x.ProductionActivityTimeType == 1921)).ToList();
                                                                        if (tempAddingPosServer.Count() > 0)
                                                                        {
                                                                            TimeSpan tmptimespanvalue = new TimeSpan();                                                                            
                                                                            TimeSpan tmpDownload_time = new TimeSpan();
                                                        ...
                                                        if (timeTrackingStage.Rework == 1)
                                                        {
                                                            ...
                                                            var DetectedStageIndex = reorderedResult1.IndexOf(timeTrackingStage);
                                                ...
                                                if (timeTrackingStage.Rework == 1)
                                                {
                                                    ...
                                                    if (lastindex - 1 > DetectedStageIndex)
                                                    {
                                                        ...
                                                        if (timeTrackingStage.IdStage == 2 && TimeTrackingListnew[i].DesignSystem == "EDS")
                                                        {
                                                            ...
                                                            var tempAddingPosServer = TimeTrackingListnew[i].TimeTrackingAddingPostServer.Where(x => x.TimeTrackIdCounterparttracking == timeTrackingStage.IdCounterparttracking && (x.ProductionActivityTimeType == 0 || x.ProductionActivityTimeType == 2228 || x.ProductionActivityTimeType == 2229 || x.ProductionActivityTimeType == 1920 || x.ProductionActivityTimeType == 1921)).ToList();
                                ...
                                #region  [pallavi jadhav][GEOS2-7066][10-04-2025]  
                                //if (timeTrackingStage.IdStage == 2)
                                //{
                                ...
                                //        if (timeTrackingStage.ProductionActivityTimeType == 2228)
                                //        {
                                //            AddintotalTimeDifference = TimeSpan.ParseExact(Convert.ToString(timeTrackingStage.TimeTrackDifference), format, null);
                                ...
                                //        }
                                //        if (timeTrackingStage.ProductionActivityTimeType == 2229)
                                //        {
                                //            PostservertotalTimeDifference = TimeSpan.ParseExact(Convert.ToString(timeTrackingStage.TimeTrackDifference), format, null);
                                ...
                                //        if (Production == Convert.ToString("Production_" + timeTrackingStage.IdStage))
                                //        {
                                //            dr[Production] = AddintotalTimeDifference + PostservertotalTimeDifference;
                                //        }
                        ...
                        //    //end
                        //    foreach (var timeTrackingStage in temprecord)
                        //    {
                        //        bool FlagPresentInActivePlants = false;
                        //        var Stagerecord = TimeTrackingProductionStage.Where(x => x.IdStage == timeTrackingStage.IdStage).FirstOrDefault();
                        //        if (Stagerecord != null)
                        ...
                        //                if (reworkflag == true)
                        //                {
                        //                    if (checkIdstage == timeTrackingStage.IdStage)
                        //                    {
                        //                        reworkflag = false;
                        //                        checkIdstage = 0;
                        //                        //Rework = "Rework_" + timeTrackingStage.IdStage;
                        ...
                        //                        //ReworkTimeIndouble = ReworkTimeIndouble + timeTrackingStage.TimeDifference;
                        //                        ReworkTime = TimeSpan.FromSeconds(Convert.ToDouble(ReworkTimeIndouble));
                        //                        TotalReworktime += ReworkTime;
                        //                        ProductionOWStime = ProductionOWStime + timeTrackingStage.TimeDifference;//rajashri GEOS2-6054
                        //                        OWSprodandReworkTime = TimeSpan.FromSeconds(Convert.ToDouble(ProductionOWStime));//rajashri GEOS2-6054
                        //                        #region
                        //                        //rajashri start GEOS2-6054
                        //                        if (timeTrackingStage.Rework > 0)
                        //                        {
                        //                            var DetectedStageIndex1 = temprecord.IndexOf(timeTrackingStage);
                        ...
                        //                        if (TempTimeTrackingByStageList.Count() > 0)
                        //                        {
                        //                            //decimal ? Real = TempTimeTrackingByStageList.Where(x => x.IdStage == timeTrackingStage.IdStage).FirstOrDefault().Real;
                        ...
                        //                            //TempTimeTrackingByStageList.Where(x => x.IdStage == timeTrackingStage.IdStage).ToList().ForEach(a => a.Real = Realtime);
                        //                            TempTimeTrackingByStageList.Where(x => x.IdStage == DetectedIdStage).ToList().ForEach(a => a.Real = Realtime);
                        //                            TempTimeTrackingByStageList.Where(x => x.IdStage == DetectedIdStage).ToList().ForEach(a => a.PlannedDeliveryDateByStage = timeTrackingStage.PlannedDeliveryDateByStage);
                        //                            TempTimeTrackingByStageList.Where(x => x.IdStage == DetectedIdStage).ToList().ForEach(a => a.PlannedDeliveryDateHtmlColor = timeTrackingStage.PlannedDeliveryDateHtmlColor);
                        //                            //dr["PlannedDeliveryDateHtmlColor"] = timeTrackingStage.PlannedDeliveryDateHtmlColor;
                        //                            TempTimeTrackingByStageList.Where(x => x.IdStage == DetectedIdStage).ToList().ForEach(a => a.Days = timeTrackingStage.Days);
                        ...
                        //                            TempTimeTrackingByStage.Real = Convert.ToDecimal(timeTrackingStage.TimeDifference);
                        //                            TempTimeTrackingByStage.PlannedDeliveryDateByStage = timeTrackingStage.PlannedDeliveryDateByStage;
                        //                            TempTimeTrackingByStage.PlannedDeliveryDateHtmlColor = timeTrackingStage.PlannedDeliveryDateHtmlColor;
                        //                            TempTimeTrackingByStage.Days = timeTrackingStage.Days;
                        //                            TempTimeTrackingByStageList.Add(TempTimeTrackingByStage);
                        ...
                        //                        {
                        //                            reworkflag = true;
                        //                            checkIdstage = timeTrackingStage.IdStage;
                        //                            var DetectedStageIndex = temprecord.IndexOf(timeTrackingStage);
                        ...
                        //                        {
                        //                            //if (checkIdstageWithinRework == timeTrackingStage.IdStage)
                        //                            //{
                        ...
                        //                            if (ReworkTimeStageList.Any(ti => ti.IdStage == timeTrackingStage.IdStage))
                        //                            {
                        //                                ReworkTimeIndouble = ReworkTimeIndouble + timeTrackingStage.TimeDifference;
                        //                            }
                        //                            else
                        //                            {
                        //                                ProductionOWStime = ProductionOWStime + timeTrackingStage.TimeDifference;
                        //                            }
                        ...
                        //                                    string format = "hh\\:mm\\:ss"; // Specify the format of the time string
                        //                                    TimeSpan TempReworkInTimespan = TimeSpan.ParseExact(timeString, format, null);
                        //                                    //TimeSpan timespan = TempReworkInTimespan + TimeSpan.FromSeconds(Convert.ToDouble(timeTrackingStage.TimeDifference));
                        ...
                        //                                        TempTimeTrackingByStageList.Where(x => x.IdStage == DetectedIdStagWithinRework).ToList().ForEach(a => a.Expected = 0);
                        //                                    }
                        //                                    decimal? Realtime = RealTime + Convert.ToDecimal(timeTrackingStage.TimeDifference);
                        //                                    //TempTimeTrackingByStageList.Where(x => x.IdStage == timeTrackingStage.IdStage).ToList().ForEach(a => a.Real = Realtime);
                        //                                    TempTimeTrackingByStageList.Where(x => x.IdStage == DetectedIdStagWithinRework).ToList().ForEach(a => a.Real = Realtime);
                        //                                    TempTimeTrackingByStageList.Where(x => x.IdStage == DetectedIdStagWithinRework).ToList().ForEach(a => a.PlannedDeliveryDateByStage = timeTrackingStage.PlannedDeliveryDateByStage);
                        //                                    TempTimeTrackingByStageList.Where(x => x.IdStage == DetectedIdStagWithinRework).ToList().ForEach(a => a.Days = timeTrackingStage.Days);
                        //                                    TempTimeTrackingByStageList.Where(x => x.IdStage == DetectedIdStagWithinRework).ToList().ForEach(a => a.PlannedDeliveryDateHtmlColor = timeTrackingStage.PlannedDeliveryDateHtmlColor);
                        ...
                        //                                    TempTimeTrackingByStageList.Add(TempTimeTrackingByStage);
                        //                                }
                        //                                if (DetectedIdStagWithinRework == timeTrackingStage.IdStage)
                        //                                {
                        //                                    ReworkTimeIndoubleWithinRework = ReworkTimeIndoubleWithinRework + timeTrackingStage.TimeDifference;
                        //                                    checkIdstageWithinRework = 0;
                        ...
                        //                            //if (timeTrackingStage.Rework == 1)
                        //                            //{
                        //                            //    checkIdstageWithinRework = timeTrackingStage.IdStage;
                        //                            //    ReworkTimeIndouble = ReworkTimeIndouble + timeTrackingStage.TimeDifference;
                        //                            //}
                        //                            //else
                        //                            //{
                        ...
                        //                                    if (ReworkTimeStageList.Any(ti => ti.IdStage == timeTrackingStage.IdStage))
                        //                                    {
                        //                                        var timeDifference = ReworkTimeStageList.Where(x => x.IdStage == timeTrackingStage.IdStage).Select(x => x.TimeDifference).FirstOrDefault();
                        //                                        var totalTimeDifference = ReworkTimeStageList.Where(x => x.IdStage == timeTrackingStage.IdStage).Sum(x => x.TimeDifference);
                        ...
                        //                            {
                        //                                //decimal ? Real = TempTimeTrackingByStageList.Where(x => x.IdStage == timeTrackingStage.IdStage).FirstOrDefault().Real;
                        //                                var TempProductionRecord = TempTimeTrackingByStageList.Where(x => x.IdStage == timeTrackingStage.IdStage).FirstOrDefault();
                        ...
                        //                                    decimal? Realtime = TempReal + Convert.ToDecimal(timeTrackingStage.TimeDifference);
                        //                                    decimal? Productiontime = TempProduction + Convert.ToDecimal(timeTrackingStage.TimeDifference);
                        //                                    //TempTimeTrackingByStageList.Where(x => x.IdStage == timeTrackingStage.IdStage).ToList().ForEach(a => a.Real = Realtime);
                        //                                    TempTimeTrackingByStageList.Where(x => x.IdStage == timeTrackingStage.IdStage).ToList().ForEach(a => a.Real = Realtime);
                        //                                    //TempTimeTrackingByStageList.Where(x => x.IdStage == timeTrackingStage.IdStage).ToList().ForEach(a => a.Production = Productiontime);
                        //                                }
                        //                                else
                        //                                {
                        //                                    TempTimeTrackingByStageList.Where(x => x.IdStage == timeTrackingStage.IdStage).ToList().ForEach(a => a.Expected = 0);
                        ...
                        //                                TempTimeTrackingByStage.IdStage = timeTrackingStage.IdStage;
                        //                                TempTimeTrackingByStage.Real = Convert.ToDecimal(timeTrackingStage.TimeDifference);
                        //                               // TempTimeTrackingByStage.Production = Convert.ToDecimal(timeTrackingStage.TimeDifference);
                        //                                TempTimeTrackingByStageList.Add(TempTimeTrackingByStage);
                        //                            }
                        //                        }
                        //                    }
                        ...
                        //                        ProductionTime = TimeSpan.FromSeconds(Convert.ToDouble(timeTrackingStage.TimeDifference));
                        //                        ReworkTime = TimeSpan.FromSeconds(Convert.ToDouble(0));
                        //                        POWS = "POWS_" + timeTrackingStage.IdStage;//rajashri GEOS2-6054
                        //                        ROWS = "ROWS_" + timeTrackingStage.IdStage;//rajashri GEOS2-6054
                        ...
                        //                                //TempTimeTrackingByStageList.Where(x => x.IdStage == timeTrackingStage.IdStage).ToList().ForEach(a => a.Real = Realtime);
                        //                                TempTimeTrackingByStageList.Where(x => x.IdStage == timeTrackingStage.IdStage).ToList().ForEach(a => a.Real = Realtime);
                        //                                TempTimeTrackingByStageList.Where(x => x.IdStage == timeTrackingStage.IdStage).ToList().ForEach(a => a.PlannedDeliveryDateByStage = timeTrackingStage.PlannedDeliveryDateByStage);
                        //                                TempTimeTrackingByStageList.Where(x => x.IdStage == timeTrackingStage.IdStage).ToList().ForEach(a => a.PlannedDeliveryDateHtmlColor = timeTrackingStage.PlannedDeliveryDateHtmlColor);
                        //                                TempTimeTrackingByStageList.Where(x => x.IdStage == timeTrackingStage.IdStage).ToList().ForEach(a => a.Days = timeTrackingStage.Days);
                        //                                // TempTimeTrackingByStageList.Where(x => x.IdStage == timeTrackingStage.IdStage).ToList().ForEach(a => a.Production = Productiontime);
                        //                            }
                        //                            else
                        //                            {
...
)
        {
            ...
            try
            {
                ...
                //TimeTrackingList.AddRange(ERMService.GetTimeTrackingBYPlant_V2630(OriginalPlantIdSite, ProductionIdSite, CurrencyNameFromSetting, GeosAppSettingList, DateTime.ParseExact(fromDate, "dd/MM/yyyy", null),DateTime.ParseExact(toDate, "dd/MM/yyyy", null)));  // [pallavi jadhav][GEOS2-7060][25-03-2025] 
                //TimeTrackingList.AddRange(ERMService.GetCamCadTimeTrackingBYPlant_V2640(OriginalPlantIdSite, ProductionIdSite, CurrencyNameFromSetting, GeosAppSettingList, DateTime.ParseExact(fromDate, "dd/MM/yyyy", null),DateTime.ParseExact(toDate, "dd/MM/yyyy", null)));  // [pallavi jadhav][GEOS2-7060][25-03-2025] 
                //TimeTrackingList.AddRange(ERMService.GetCamCadTimeTrackingBYPlant_V2660(OriginalPlantIdSite, ProductionIdSite, CurrencyNameFromSetting, GeosAppSettingList, DateTime.ParseExact(fromDate, "dd/MM/yyyy", null), DateTime.ParseExact(toDate, "dd/MM/yyyy", null)));  // [GEOS2-8868][gulab lakade][10 07 2025] 
                TimeTrackingList.AddRange(ERMService.GetCamCadTimeTrackingBYPlant_V2670(OriginalPlantIdSite, ProductionIdSite, CurrencyNameFromSetting, GeosAppSettingList, DateTime.ParseExact(fromDate, "dd/MM/yyyy", null), DateTime.ParseExact(toDate, "dd/MM/yyyy", null)));  // [GEOS2-8309][rani dhamankar][29 08 2025]
                        ...
                        if (!templateWithTimeTracking.TimeTracking.Contains(TempDate))
                        {
                            ...
                            if (DeliveryWeekcount == 0)
                            {
                                if (TemplateWithTimeTracking.Where(a => a.deliwaryWeek.Contains(item.DeliveryWeek)).Count() == 0)
                                {
                                    ...
                                    //templateWithTimeTracking.deliwaryWeek = timeTrackingStage.DeliveryWeek + " (" + Updateitem.TimeTracking.Count() + ")";
                                    //templateWithTimeTracking.TimeTracking = new List<string>();
