...
{
    /// <summary>
    /// [000][cpatil][09-05-2022][GEOS2-3749] Display X and V OT's - Edit ELECTRIFICATION WorkOrder
    /// </summary>
    public class WorkOrderItemDetailsForElectrificationViewModel : NavigationViewModelBase, INotifyPropertyChanged, IDisposable
    {
        #region Task log
        //
        #endregion
        ...
        private WarehouseProduct selectedWarehouseProduct;
        private bool isDeleted;
        private ObservableCollection<OtItem> updatedOtItemList;
        ...
        private bool seeAllArticles;
        private TaskCompletionSource<bool> _articleLoadingCompleted = new TaskCompletionSource<bool>();
        #endregion // End Of Declaration
        #region Public Properties
        public List<Tuple<long, decimal>> OtItemQuntity { get; set; }
        public ObservableCollection<OtItem> UpdatedOtItemList
        {
            get { return updatedOtItemList; }
            set
            {
                updatedOtItemList = value;
                OnPropertyChanged(new PropertyChangedEventArgs("UpdatedOtItemsList"));
            }
        }
        public ObservableCollection<OtItem> DeletedOtItemList
        {
            get { return deletedOtItemList; }
            set
            {
                deletedOtItemList = value;
                OnPropertyChanged(new PropertyChangedEventArgs("DeletedOtItemList"));
            }
        }
        public ObservableCollection<OtItem> AddedOtItemList
        {
            get { return addedOtItemList; }
            set
            {
                addedOtItemList = value;
                OnPropertyChanged(new PropertyChangedEventArgs("AddedOtItemList"));
            }
        }
        ...
        public WarehouseProduct SelectedWarehouseProduct
        {
            get
            {
                return selectedWarehouseProduct;
            }
            set
            {
                selectedWarehouseProduct = value;
                OnPropertyChanged(new PropertyChangedEventArgs("SelectedWarehouseProduct"));
            }
        }
        #endregion
        #region ICommands
        ...
        public ICommand AddReferenceButtonCommand { get; set; } //[pramod.misal][08.07.2024][GEOS-5473]
        ...
        #region Constructor
        public WorkOrderItemDetailsForElectrificationViewModel()
        {
            try
            {
                GeosApplication.Instance.Logger.Log("Constructor WorkOrderItemDetailsForElectrificationViewModel()...", category: Category.Info, priority: Priority.Low);
                IsCancel = false;
                ...
                //[pramod.misal][08.07.2024][GEOS-5473]
                AddReferenceButtonCommand = new RelayCommand(new Action<object>(AddReferenceButtonAction));
                EditCommand = new RelayCommand(new Action<object>(EditItemAction));
                DeleteItemCommand = new RelayCommand(new Action<object>(DeleteItemCommandAction));
                GeosApplication.Instance.Logger.Log("Constructor WorkOrderItemDetailsForElectrificationViewModel()....executed successfully", category: Category.Info, priority: Priority.Low);
            }
            catch (Exception ex)
            {
                GeosApplication.Instance.Logger.Log(string.Format("Error in Constructor WorkOrderItemDetailsForElectrificationViewModel() - {0}", ex.Message), category: Category.Exception, priority: Priority.Low);
            }
        }
        //[nsatpute][29.07.2024][GEOS2-5473]
        public async Task GetAllreferences()
        {
            try
            {
                GeosApplication.Instance.Logger.Log("Constructor GetAllreferences()...", category: Category.Info, priority: Priority.Low);
                List<Article> articles = await Task.Run(() => SAMService.GetAllReference(seeAllArticles));
                Application.Current.Dispatcher.Invoke(() =>
                {
                    referenceList = new ObservableCollection<Article>(articles);
                    _articleLoadingCompleted.SetResult(true); // Signal that loading is complete
                });
                GeosApplication.Instance.Logger.Log("Constructor GetAllreferences()....executed successfully", category: Category.Info, priority: Priority.Low);
            }
            catch (Exception ex)
            {
                GeosApplication.Instance.Logger.Log(string.Format("Error in Constructor GetAllreferences() - {0}", ex.Message), category: Category.Exception, priority: Priority.Low);
                _articleLoadingCompleted.SetException(ex);
            }
        }
        ...
        public void Init(Ots ot)
        {
            try
            {
                ...
                OT = SAMService.GetWorkOrderByIdOt_V2540(ot.IdOT, ot.Site);
                //[rdixit][GEOS2-5474][26.07.2024]
                OtItemQuntity = new List<Tuple<long, decimal>>();
                OT?.OtItems.ForEach(i => OtItemQuntity.Add(new Tuple<long, decimal>(i.IdOTItem, i.RevisionItem.Quantity)));
                OT.OTLogEntries = new List<LogEntriesByOT>(OT.OTLogEntries?.OrderByDescending(i => i.Datetime));
                //[adhatkar][GEOS2-2961]
                Remark = OT.Observations;
                //SelectedOtItem = OT.OtItems.FirstOrDefault();
                //[pramod.misal][GEOS2-5473][15.07.2024]
                SelectedOtItem = OT.OtItemsOb.FirstOrDefault();
                //OT.OtItemsOb.ParentId
                ...
                if ((OT.Quotation.Site.Customer.IdCustomer != 10) && ((OT.Quotation.Site.Customer.IdCustomer != 8)))
                    seeAllArticles = false;
                else
                    seeAllArticles = true;
                //GetAllreferences();
        ...
        private void SaveWorkOrderDetails(object obj)
        {
            try
            {
                if (!string.IsNullOrEmpty(Remark))
                    Remark = Remark.Trim();
                List<LogEntriesByOT> LogEntriesByOTList = new List<LogEntriesByOT>();
                //ADDED OtItem  //[rdixit][GEOS2-5474][26.07.2024]
                if (OT.AddedOtItemList != null)
                {
                    OT.AddedOtItemList.ToList().ForEach(item => item.RevisionItem.WarehouseProduct.Article.ArticleImageInBytes = null);
                    OT.AddedOtItemList.ToList().ForEach(item => item.RevisionItem.WarehouseProduct.Article.ArticleImage = null);
                    OT.AddedOtItemList.ToList().ForEach(item => item.RevisionItem.IdRevision = OT.OtItems[0].IdRevision);
                    foreach (var item in OT.AddedOtItemList?.Where(i => i.ParentId < 1)?.ToList())
                    {
                        LogEntriesByOT logEntriesByOT_1 = new LogEntriesByOT();
                        logEntriesByOT_1.IdOT = OT.IdOT;
                        logEntriesByOT_1.IdUser = GeosApplication.Instance.ActiveUser.IdUser;
                        logEntriesByOT_1.Comments = string.Format(System.Windows.Application.Current.FindResource("OTItemAddlogmsg").ToString(),
                            item.RevisionItem.WarehouseProduct.Article.Reference);
                        logEntriesByOT_1.IdLogEntryType = 1;
                        logEntriesByOT_1.IsRtfText = false;
                        logEntriesByOT_1.Datetime = GeosApplication.Instance.ServerDateTime;
                        logEntriesByOT_1.People = new People();
                        logEntriesByOT_1.People.IdPerson = GeosApplication.Instance.ActiveUser.IdUser;
                ...
                if (OT.UpdatedOtItemList != null) //[rushikesh.gaikwad][GEOS2-5473][18.07.2024]
                {
                    OT.UpdatedOtItemList.ToList().ForEach(item => item.RevisionItem.WarehouseProduct.Article.ArticleImageInBytes = null);
                    OT.UpdatedOtItemList.ToList().ForEach(item => item.RevisionItem.WarehouseProduct.Article.ArticleImage = null);
                    foreach (var item in OT.UpdatedOtItemList)
                    {
                        if (OtItemQuntity != null)
                        {
                            if (OtItemQuntity.Any(i => i.Item1 == item.IdOTItem))
                            {
                                var Old = OtItemQuntity.FirstOrDefault(i => i.Item1 == item.IdOTItem);
                                if (Old.Item2 != item.RevisionItem?.Quantity)
                                {
                                    LogEntriesByOT logEntriesByOT_1 = new LogEntriesByOT();
                                    logEntriesByOT_1.IdOT = OT.IdOT;
                                    logEntriesByOT_1.IdUser = GeosApplication.Instance.ActiveUser.IdUser;
                                    logEntriesByOT_1.Comments = string.Format(Application.Current.FindResource("OTItemQuantityUpdate").ToString(),
                                        item.RevisionItem.WarehouseProduct.Article.Reference, Old.Item2, item.RevisionItem?.Quantity);
                                    logEntriesByOT_1.IdLogEntryType = 1;
                                    logEntriesByOT_1.IsRtfText = false;
                                    logEntriesByOT_1.Datetime = GeosApplication.Instance.ServerDateTime;
                                    logEntriesByOT_1.People = new People();
                ...
                #region old service commited
                //SAMService.UpdateOTAssignedUser_V2180(objOT.Site, null, OT.IdOT, ot.Observations, Remark, GeosApplication.Instance.ActiveUser.IdUser);
                //New
                //SAMService = new SAMServiceController("localhost:6699");
                //[pramod.misal][GEOS2-5473][15.07.2024]
                #endregion
                SAMService.UpdateOTAssignedUser_V2540(objOT.Site, null, OT.IdOT, ot.Observations, Remark, GeosApplication.Instance.ActiveUser.IdUser, OT.DeletedOtItemList?.ToList(), OT.AddedOtItemList?.ToList(), OT.UpdatedOtItemList?.ToList(), LogEntriesByOTList);
                if (OT.OTLogEntries == null)
                    OT.OTLogEntries = new List<LogEntriesByOT>();
                if (LogEntriesByOTList != null)
                {
                    OT.OTLogEntries.AddRange(LogEntriesByOTList);
                    OT.OTLogEntries = new List<LogEntriesByOT>(OT.OTLogEntries.OrderByDescending(i => i.Datetime));
                }
        ...
        private void ImageClickCommandAction(object obj)
        {
            ...
            try
            {
                ...
                #endregion
                //[pramod.misal][GEOS2-5473][15.07.2024]
                OT.OtItemsOb.Where(a => a.IdOTItem != otItem.IdOTItem && a.ShowComment == true).ToList().ForEach(a => { a.ShowComment = false; });
                OT.OtItemsOb.Where(a => a.IdOTItem == otItem.IdOTItem).ToList().ForEach(a => { a.ShowComment = !a.ShowComment; });
            }
            catch (Exception ex)
            {
        ...
        #endregion
        //[pramod.misal][08.07.2024][GEOS-5473]
        private void AddReferenceButtonAction(object obj)
        {
            try
            {
                GeosApplication.Instance.Logger.Log("Method AddReferenceButtonAction ...", category: Category.Info, priority: Priority.Low);
                DXSplashScreen.Show<SplashScreenView>();
                if (DXSplashScreen.IsActive && !GeosApplication.Instance.IsLoadOneTime) { DXSplashScreen.Close(); }
                IsBusy = true;
                if (GeosApplication.Instance.SamWorkOrderArticles == null)
                {
                    GeosApplication.Instance.SamWorkOrderArticles = new List<Article>();
                    GeosApplication.Instance.SamWorkOrderArticles = SAMService.GetAllReference(seeAllArticles);
                    referenceList = new ObservableCollection<Article>(GeosApplication.Instance.SamWorkOrderArticles);
                }
                else
                {
                    referenceList = new ObservableCollection<Article>(GeosApplication.Instance.SamWorkOrderArticles);
                }
                AddEditReferenceView addEditReferenceView = new AddEditReferenceView();
                AddEditReferenceViewModel addEditReferenceViewModel = new AddEditReferenceViewModel(referenceList);
                EventHandler handle = delegate { addEditReferenceView.Close(); };
                addEditReferenceViewModel.RequestClose += handle;
                addEditReferenceViewModel.WindowHeader = System.Windows.Application.Current.FindResource("AddItemHeader").ToString();
                addEditReferenceViewModel.IsNew = true;
                addEditReferenceViewModel.Init(OT.OtItemsOb);
                addEditReferenceView.DataContext = addEditReferenceViewModel;
                addEditReferenceView.ShowDialog();
                if (addEditReferenceViewModel.IsNew)
                {
                    if (OT.OtItemsOb == null)
                        OT.OtItemsOb = new ObservableCollection<OtItem>();
                    if (AddedOtItemList == null)
                        AddedOtItemList = new ObservableCollection<OtItem>();
                    AddedOtItemList.Add(addEditReferenceViewModel.NewOtItem);
                    AddComponentArticles(addEditReferenceViewModel.NewOtItem);
                    AddedOtItemList.ToList().ForEach(item => item.RevisionItem.WarehouseProduct.Article.ArticleImageInBytes = null);
                    AddedOtItemList.ToList().ForEach(item => item.RevisionItem.WarehouseProduct.Article.ArticleImage = null);
                    if (OT.AddedOtItemList == null)
                    {
                        OT.AddedOtItemList = new ObservableCollection<OtItem>();
                        OT.AddedOtItemList = AddedOtItemList;
                    }
                    foreach (OtItem oi in OT.AddedOtItemList)
                    {
                        if (!OT.OtItemsOb.Any(x => x.RevisionItem.NumItem == oi.RevisionItem.NumItem))
                            OT.OtItemsOb.Add(oi);
                    }
                }
                IsBusy = false;
                if (DXSplashScreen.IsActive) { DXSplashScreen.Close(); }
                GeosApplication.Instance.Logger.Log("Method AddReferenceButtonAction()....executed successfully", category: Category.Info, priority: Priority.Low);
            }
            catch (Exception ex)
            {
                IsBusy = false;
        ...
        private void AddComponentArticles(OtItem otitem)
        {
            List<Article> componentArticles = SAMService.GetComponentArticlesByIdArticle(otitem.RevisionItem.WarehouseProduct.Article.IdArticle);
            OtItem decomposition; int numItemCounter = 0;
            foreach (Article component in componentArticles)
            {
                numItemCounter++;
                var lstrecord = AddedOtItemList.LastOrDefault();
                int lstparatid = lstrecord.ParentId;
                int lastkeyid = lstrecord.KeyId;
                decomposition = new Data.Common.OtItem();
                decomposition.RevisionItem = new RevisionItem();
                decomposition.ParentId = otitem.KeyId;
                decomposition.RevisionItem.NumItem = $"{otitem.RevisionItem.NumItem}.{numItemCounter}";
                decomposition.KeyId = lastkeyid + 1;
                decomposition.RevisionItem.Quantity = Math.Round(Convert.ToDecimal(component.Quantity) * otitem.RevisionItem.Quantity, 4);
                decomposition.Status = new ItemOTStatusType();
                decomposition.Status.Name = "";
                decomposition.RevisionItem.WarehouseProduct = new WarehouseProduct();
                decomposition.RevisionItem.WarehouseProduct.Article = component;
                AddedOtItemList.Add(decomposition);
            }
        }
        private void EditItemAction(object obj)
        {
            try
            {
                ...
                DevExpress.Xpf.Grid.TreeListView tableView = (DevExpress.Xpf.Grid.TreeListView)obj;
                var ownerInfo = (tableView as FrameworkElement);
                OtItem SelectedOtItem = (OtItem)tableView.DataControl.CurrentItem;
                if (SelectedOtItem.Status.Name == "READY" || SelectedOtItem.Status.Name == "STAND BY")
                {
                    DXSplashScreen.Show<SplashScreenView>();
                    if (DXSplashScreen.IsActive && !GeosApplication.Instance.IsLoadOneTime) { DXSplashScreen.Close(); }
                    IsBusy = true;
                    ...
                    AddEditReferenceView addEditReferenceView = new AddEditReferenceView();
                    AddEditReferenceViewModel addEditReferenceViewModel = new AddEditReferenceViewModel(referenceList);
                    addEditReferenceViewModel.OldQuantity = SelectedOtItem.RevisionItem.Quantity;
                    if (!DXSplashScreen.IsActive) { DXSplashScreen.Show<SplashScreenView>(); }
                    EventHandler handle = delegate { addEditReferenceView.Close(); };
                    addEditReferenceViewModel.RequestClose += handle;
                    addEditReferenceViewModel.IsNew = false;
                    addEditReferenceViewModel.WindowHeader = System.Windows.Application.Current.FindResource("EditItemHeader").ToString();
                    addEditReferenceView.DataContext = addEditReferenceViewModel;
                    if (DXSplashScreen.IsActive) { DXSplashScreen.Close(); }
                    addEditReferenceView.Owner = Window.GetWindow(ownerInfo);
                    addEditReferenceViewModel.EditInitItem(SelectedOtItem);
                    addEditReferenceView.ShowDialog();
                    if (addEditReferenceViewModel.SelectedReference != null)
                    {
                        if (OT.OtItemsOb == null)
                            OT.OtItemsOb = new ObservableCollection<OtItem>();
                        var selectedItem = OT.OtItemsOb.FirstOrDefault(x => x.IdOTItem == SelectedOtItem.IdOTItem);
                        if (selectedItem != null)
                        {
                            bool referenceChanged = false;
                            if (selectedItem.RevisionItem.WarehouseProduct.Article.Reference != addEditReferenceViewModel.UpdatedOtItem.RevisionItem.WarehouseProduct.Article.Reference)
                                referenceChanged = true;
                            decimal quantityRatio = selectedItem.RevisionItem.Quantity / addEditReferenceViewModel.OldQuantity;
                            selectedItem.RevisionItem.Quantity = addEditReferenceViewModel.UpdatedOtItem.RevisionItem.Quantity;
                            selectedItem.RevisionItem.WarehouseProduct.Article.Reference = addEditReferenceViewModel.UpdatedOtItem.RevisionItem.WarehouseProduct.Article.Reference;
                            selectedItem.RevisionItem.WarehouseProduct.Article.IdArticle = addEditReferenceViewModel.UpdatedOtItem.RevisionItem.WarehouseProduct.Article.IdArticle;
                            selectedItem.RevisionItem.WarehouseProduct = addEditReferenceViewModel.UpdatedOtItem.RevisionItem.WarehouseProduct;
                            if (UpdatedOtItemList == null)
                                updatedOtItemList = new ObservableCollection<OtItem>();
                            UpdatedOtItemList.Add(selectedItem);
                            OT.UpdatedOtItemList = UpdatedOtItemList;  //[rushikesh.gaikwad][GEOS2-5473][18.07.2024]
                            var UpdatedOtItems = new ObservableCollection<OtItem>(OT.OtItemsOb);
                            OT.OtItemsOb = UpdatedOtItems;
                            OT.OtItemsOb.ToList().ForEach(x =>
                            {
                                if (x.RevisionItem.NumItem.Contains(selectedItem.RevisionItem.NumItem + "."))
                                {
                                    if (referenceChanged)
                                    {
                                        if (DeletedOtItemList == null)
                                            DeletedOtItemList = new ObservableCollection<OtItem>();
                                        DeletedOtItemList.Add(x);
                                        OT.OtItemsOb.Remove(x);
                                    }
                                    else
                                    {
                                        decimal calculatedQuantity = Math.Round(x.RevisionItem.Quantity * quantityRatio, 4);
                                        if (calculatedQuantity % 1 == 0)
                                            calculatedQuantity = decimal.Truncate(calculatedQuantity);
                                        x.RevisionItem.Quantity = calculatedQuantity;
                                        UpdatedOtItemList.Add(x);
                                    }
                                }
                            });
                            if (referenceChanged)
                                AddComponentArticles(selectedItem);
                        }
                    }
        ...
        private void DeleteItemCommandAction(object obj)
        {
            try
            {
                ...
                if (SelectedOtItem != null)
                {
                    SelectedWarehouseProduct = SelectedOtItem.RevisionItem.WarehouseProduct;
                    MessageBoxResult MessageBoxResult = CustomMessageBox.Show(String.Format(Application.Current.Resources["DeleteItemMessage"].ToString()), Application.Current.Resources["PopUpDeleteColor"].ToString(), CustomMessageBox.MessageImagePath.Question, MessageBoxButton.YesNo);
                    if (MessageBoxResult == MessageBoxResult.Yes)
                    {
                        if (DeletedOtItemList == null)
                            DeletedOtItemList = new ObservableCollection<OtItem>();
                        OtItem oi = SelectedOtItem;
                        DeletedOtItemList.Add(oi);
                        DeletedOtItemList.ToList().ForEach(item => item.RevisionItem.WarehouseProduct.Article.ArticleImageInBytes = null);
                        DeletedOtItemList.ToList().ForEach(item => item.RevisionItem.WarehouseProduct.Article.ArticleImage = null);
                        if (OT.DeletedOtItemList == null)
                        {
                            OT.DeletedOtItemList = new ObservableCollection<OtItem>();
                            OT.DeletedOtItemList = DeletedOtItemList;
                        }
                        foreach (var item in OT.OtItemsOb.ToList())
                        {
                            if (item.IdOTItem == oi.IdOTItem)
                            {
                                if (oi.ParentId == -1 && item.IdOTItem == oi.IdOTItem)
                                {
                                    OT.OtItemsOb.Remove(item);
                                    foreach (var sunItem in OT.OtItemsOb.ToList())
                                    {
                                        if (sunItem.IdOTItem == oi.IdOTItem)
                                        {
                                            OT.OtItemsOb.Remove(sunItem);
                                        }
                                    }
                                }
                                else if (item.IdOTItem == oi.IdOTItem && item.KeyId == oi.KeyId)
                                {
                                    OT.OtItemsOb.Remove(item);
                                    break;
                                }
                        ...
                        addedOtItemList = new ObservableCollection<OtItem>();
                        OT.OtItemsOb = new ObservableCollection<OtItem>(OT.OtItemsOb);
                        IsDeleted = true;
                    }
                }
            ...
            {
                if (DXSplashScreen.IsActive) { DXSplashScreen.Close(); }
                GeosApplication.Instance.Logger.Log("Get an error in DeleteItemCommandAction() Method " + ex.Detail.ErrorMessage, category: Category.Info, priority: Priority.Low);
                CustomMessageBox.Show(GeosApplication.Instance.ExceptionHandlingOperationString(ex.Detail, null), Application.Current.Resources["PopUpWarningColor"].ToString(), CustomMessageBox.MessageImagePath.NotOk, MessageBoxButton.OK);
            }
            catch (ServiceUnexceptedException ex)
            {
                if (DXSplashScreen.IsActive) { DXSplashScreen.Close(); }
                GeosApplication.Instance.Logger.Log("Get an error in DeleteItemCommandAction() Method - ServiceUnexceptedException " + ex.Message, category: Category.Info, priority: Priority.Low);
                GeosApplication.Instance.ExceptionHandlingOperation(ex.ExceptionType, GeosApplication.Instance.ApplicationSettings["ServicePath"].ToString(), null);
            }
            catch (Exception ex)
            {
                GeosApplication.Instance.Logger.Log("Get an error in Method DeleteItemCommandAction()...." + ex.Message, category: Category.Exception, priority: Priority.Low);
            }
        }
    }
}